...
As logistic regression is only able to distinguish between two different classes, you should use 
\href{https://en.wikipedia.org/wiki/Multiclass_classification#One-vs.-rest}{one-versus-rest} classification:
You have to build three different classifiers:
\begin{itemize}
...
\end{itemize}
Given a feature vector $\mathbf{x}$ for an iris plant, you calculate three different probabilities.
\begin{enumerate}
\item The probability $P(\mathtt{setosa}\mid \mathbf{x})$ that the given iris is an iris setosa.
\item The probability $P(\mathtt{virginica}\mid \mathbf{x})$ that the given iris is an iris virginica.
\item The probability $P(\mathtt{versicolor}\mid \mathbf{x})$ that the given iris is an iris versicolor.
\end{enumerate}
If for a given value of the feature vector $\textbf{x}$ there is a class 
$c \in \{\mathtt{setosa}, \mathtt{virginica}, \mathtt{versicolor}\}$ such that
\\[0.2cm]
\hspace*{1.3cm}
$P(c \mid \mathbf{x}) \geq\frac{1}{2}$ \quad while $P(d \mid \mathbf{x}) < \frac{1}{2}$ for all 
$d \in\{\mathtt{setosa}, \mathtt{virginica}, \mathtt{versicolor}\} \backslash \{c\}$,
\\[0.2cm]
then the class associated with the feature vector $\mathbf{x}$ is $c$, otherwise the class is $\mathtt{unkown}$.
Your task is to implement this classifier and to estimate its accuracy.  To this end,
...
accuracy of your model.  If a flower is classified as $\mathtt{unknown}$, then this should only counts as 
two thirds of a mistake, i.e.~if your classifier would classify every flower as $\mathtt{unknown}$, then its
accuracy would be $33.\bar{3}\%$.
\eox
