...
In a constraint satisfaction problem we are given a set of \blue{formul\ae} and search for values that can be
...
\item The simplest algorithm to solve a constraint satisfaction problem is via \blue{brute force search}.
      The idea behind brute force search is to test all possible \blue{variable assignments}.
...
problem, we will use the following set of variables:  \index{8 queens puzzle}
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{enumerate}
\item We have constraints that express that no two queens that are positioned in different rows share the same column.
      ...
      \\[0.2cm]
      Here the condition $j < i$ ensures that, for example, we have the constraint $\texttt{V}_2 \not= \texttt{V}_1$
...
\myFig{N-Queens-Problem-CSP.ipynb} shows a \textsc{SetlX} program that can be used to create the eight queens puzzle as a
\ac{csp}.  The code shown in this figure is more general than necessary.  Given a natural
...
like the one produced by the program shown in Figure \ref{fig:N-Queens-Problem-CSP.ipynb} and that is then capable of
