...
\item $\texttt{Constraints}$ is a set of formul\ae\ from \blue{first order logic}.  Each of these formul\ae\ is
...
\begin{enumerate}
\item $\texttt{NewValues}$ is a copy of the dictionary $\texttt{ValuesPerVar}$.
      We need to make a copy of this dictionary since we want to compute the number of values
      that have to be removed from $\texttt{ValuesPerVar[y]}$ for variables $\texttt{y}$ that are different
      from $\texttt{x}$ when $\texttt{x}$ is bound to $\texttt{value}$.  However, as we are not
      yet sure whether we really want to bind  $\texttt{x}$ to $\texttt{value}$, we can not remove values
      from the set $\texttt{ValuesPerVar}[y]$.  Instead, we remove these values from the copy
      \texttt{NewValues} of the dictionary $\texttt{ValuesPerVar}$.  This way, we do not have to
      undo any removals of values.
...
\item Now we have to find those values $\texttt{w}$ that can be assigned to $\texttt{y}$ without violating the
      constraint $\texttt{f}$.  Each time we find a value $\texttt{w}$ from $\texttt{NewValues[y]}$ such that
      the assignment
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      has to be removed from $\texttt{NewValues[y]}$.  If the constraint $\texttt{f}$ is not violated,
...
\item When we are done with the constraint \texttt{f}, we update \texttt{NewValues[y]}.
      This is necessary since there might be other constraints involving the variable $\texttt{y}$ which require
      that values have to be removed from \texttt{NewValues[y]}.  These might well be the same values that have
      already been found to be inconsistent when the constraint \texttt{f} was checked.  As we do not want to
      count these values twice, we have to update the set \texttt{NewValues[y]}.
\end{enumerate}
