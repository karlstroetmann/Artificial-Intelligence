...
The procedure $\texttt{backtrack\_search}$ shown in \myFig{csp-constraint-propagation.stlx:bt_search} is called with a
...
\item Finally, the partial variable $\texttt{Assignment}$ is updated to include the assignment of 
      $\texttt{val}$ to $\texttt{x}$ and the recursive call to $\texttt{backtrack\_search}$ tries to complete this new
      assignment. 
\end{enumerate}
