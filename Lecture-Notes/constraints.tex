...
\begin{enumerate}
\item The function $\texttt{solve}$ takes one parameter $\texttt{n}$, which is the size of the chess board.  If
...
just one optimal column for this row.  In this case, the function $\texttt{solve}$ would loop forever.  To
...
The function $\texttt{numConficts}$ shown in \myFig{numConficts.stlx} implements the function
