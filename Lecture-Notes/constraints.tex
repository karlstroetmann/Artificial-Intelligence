...
\item Before the search is started, we create a dictionary $\texttt{ValuesPerVar}$.  For every variable
      $\texttt{x}$,  the set $\texttt{ValuesPerVar[x]}$ contains those values that can be assigned to
      $\texttt{x}$ without violating a constraint. 
\item As long as the problem is not solved,  we choose a variable $\texttt{x}$. 
      This variable is chosen using the \blue{most constrained variable} heuristic:  We choose a variable
