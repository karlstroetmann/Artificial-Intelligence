...
\end{figure}
Later, when we implement functions to solve  \ac{csp}s, we will represent variable assignments and partial
...
\end{minted}
\vspace*{-0.3cm}
\caption{The eight queens problem as \ac{csp}.}
...
as a viable general purpose programming language, constraint programming has
...
The function $\texttt{solve}$ gets a constraint satisfaction problem $P$ as its input.  
...
When I tested this brute force search with the eight queens problem, it took about 30 seconds to
...
that employs backtracking.  \index{backtracking}
...
\end{enumerate}
Next, we discuss the implementation of the function $\texttt{backtrack\_search}$.  This function receives a partial assignment
...
shown in \ref{fig:is_consistent.py}.  This function takes a variable $\texttt{var}$, a $\texttt{value}$, a partial 
...
\begin{enumerate}
\item The function $\texttt{solve}$ receives a $\texttt{CSP}$.  This $\texttt{CSP}$ is first split into its
...
\item Otherwise, we choose a variable $\texttt{x}$ such that the number of values that can still be used to
      instantiate $\texttt{x}$ is minimal.  This variable is computed using the function
...
\myFig{Constraint-Propagation-Solver.ipynb:most_constrained_variable} show the implementation of the function
...
\item Finally, the first variable \texttt{x} that has only \texttt{minSize} values available is returned.
\end{enumerate}
...
\noindent
The function $\texttt{propagate}$ shown in \myFig{Constraint-Propagation-Solver.ipynb:propagate} takes the following inputs:
\begin{enumerate}[(a)]
...
30 milliseconds to find a solution.  I have also tested it with the Zebra Puzzle described in the next exercise.
...
If we try to solve the cryptarithmetic puzzle as coded in \myFig{Crypto-Arithmetic.ipynb} using the
constraint solver developed in the previous section, we will be disappointed: Solving the puzzle takes about
half a second on my computer.  The reason is that most constraints involve either three or four variables and
therefore the effects of constraint propagation kick only in when many variables have already been initialized.
However, we can solve the problem in less than 70 milliseconds if we add the following constraints for the 
...
\item As long as the set $\texttt{UncheckedVariables}$ is not empty, we remove one variable $x$ from this set:
      \\[0.2cm]
      \hspace*{1.3cm}
      $x := \texttt{UncheckedVariables}.\mathtt{pop}()$
...
\item $\texttt{Annotated}$ is a dictionary mapping constraints to the set of variables occurring in them, 
      i.e.~if $f$ is a constraint, then $\mathtt{Annotated}[f]$ is the set of variables
      occurring in $f$.
...
\item Otherwise, the set of values that are known to be consistent for \texttt{var} is stored
      as \\ $\mathtt{ValuesPerVar}[\mathtt{var}]$.
\end{enumerate}
...
\begin{enumerate}
\item As the argument $\mathtt{Variables}$ is a \texttt{frozenset} but we need to modify this set for the
...
\item Rinse and repeat until a solution is found or the sun rises in the west.
\end{enumerate}
...
\noindent
\myFig{Local-Search.ipynb:solve} shows an implementation of these ideas in \textsl{Python}.  We proceed to discuss
this program line by line.
\begin{enumerate}
\item The function $\texttt{solve}$ takes one parameter $P$, which is a \ac{csp}.  If
      the computation is successful, $\texttt{solve(P)}$ returns a dictionary that encodes a solution of the
      given \ac{csp} by mapping variables to values.
\item The sets \texttt{Variables} and \texttt{Values} are turned to lists.  This is necessary because the
      function
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      a number $k \in \{0, \cdots, \mathtt{len}(L)-1\}$ the expression $L[k]$ needs to be defined.
\item \texttt{Annotated} is a list of pairs of the form $(f, V)$ where $f$ is a constraint and $V$ is the set
       of variables occurring in $f$.
\item \texttt{Assign} is a dictionary mapping all variables form the set \texttt{Variables} to values from the
...
\item If we have reassigned a variable $x$ in the last iteration of the loop, then we do not want to reassign
      it again.  Therefore, the variable \texttt{lastVar} stores the variable that has been reassigned in the
      previous iteration.
\item At the beginning of the \texttt{while} loop, we count the number of conflicts for all variables, i.e.~if
      ...
      The list \texttt{Conflicts} defined in line 10 contains pairs of the form $(n, x)$ where $x$ is a
      variable and $n$ is the number of conflicts that this variable is involved in.
\item In line 13 the list \texttt{Conflicts} is turned into a set that is represented as an ordered binary set.
...
\item Otherwise, the list  \texttt{FaultyVars} defined in line 19 collects those variables that have a maximal
      number of conflicts.  
\item In line \texttt{20} we choose a random variable \texttt{x} from this set as the variable to be
      reassigned.  However, this is only done nine out of ten times.  In order to avoid running into an
      infinite where we keep changing the same variables, every tenth iteration instead of choosing \texttt{x}
      as the variable violating the most constraints we choose \texttt{x} randomly.  This is controlled by the
      test \texttt{iteration \% 10 == 0} in line 16.
\item Line 21 computes a list \texttt{Conflicts} that this time contains pairs of the form $(n, v)$ where $n$ 
      is the number of conflicts that the variable \texttt{x} would cause if we would assign the value $v$ to
      \texttt{x}.
\item Line 27 casts the list \texttt{Conflicts} into a set that is represented as an ordered binary set.
...
\item \texttt{newVal} is a random element from this list that is then assigned to \texttt{x}.
      Again, this is only done 9 out of ten times.  The tenth time a random value is assigned to \texttt{x}.
\item In line 31 we remember that we have reassigned \texttt{x} in this iteration so that we don't reassign
...
$\mathtt{numConficts}(x,\mathtt{Assign}, \mathtt{Annotated})$
...
tackled if sufficient time is available.  Hence, it seems that local search is better than the algorithms
