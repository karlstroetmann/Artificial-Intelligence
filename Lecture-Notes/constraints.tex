...
In a \blue{constraint satisfaction problem}, we are presented with a set of \blue{formulas}, and our objective
is to find \blue{values} that can be assigned to the \blue{variables} in these formulas, ensuring all formulas
...
structure for guiding the search, \blue{constraint satisfaction problems} feature structured states in the form
of \blue{variable assignments}.  This structure can be leveraged to direct the search process more effectively.

This chapter is organized as follows:
\begin{enumerate}[(a)]
\item The initial section introduces the concept of a constraint satisfaction problem. To elucidate this
  concept, we explore two examples: \blue{map colouring} and the \blue{eight queens puzzle}. Subsequently, we
  examine various applications of constraint satisfaction problems. 
\item The most basic algorithm for addressing a constraint satisfaction problem is \blue{brute force
    search}. The principle of \emph{brute force search} involves examining every possible \blue{variable
    assignment}. 
\item Often, the search space is so vast that enumerating all variable assignments becomes
  impractical. \blue{Backtracking search} enhances brute force search by intertwining the generation of
  variable assignments with the evaluation of constraints, significantly boosting the efficiency of the
  search. 
\item Backtracking search can be further refined through the integration of \blue{constraint propagation} and
  the application of the \blue{most restricted variable} heuristic. 
\item Additionally, verifying the \blue{consistency} of values assigned to different variables can
  substantially reduce the search space. 
\item \blue{Local search} presents an alternative methodology for solving constraint satisfaction problems, particularly beneficial for large but uncomplicated problems.
\item Lastly, we discuss the \href{https://microsoft.github.io/z3guide/}{\texttt{Z3}} theorem prover, an industrial-grade \blue{constraint solver}. Here, a \emph{constraint solver} is defined as software that accepts a \emph{constraint satisfaction problem} as input and produces a \emph{solution} for the problem.
\end{enumerate}

Upon concluding our exploration of constraint satisfaction problems, we will have developed a constraint solver capable of resolving the most challenging \href{https://en.wikipedia.org/wiki/Sudoku}{Sudoku} puzzles in mere seconds.
...
\begin{enumerate}[(a)]
\item $\texttt{Vars}$ represents a set of strings, functioning as \blue{variables}.
\item $\texttt{Values}$ denotes a set of \blue{values} that can be assigned to the variables in $\texttt{Vars}$.
\item $\texttt{Constraints}$ is a collection of formulas derived from \blue{first order logic}, each termed a
      \blue{constraint} of $\mathcal{P}$. To evaluate these formulas, an \blue{interpretation} of the function and
...
\end{enumerate}
In subsequent sections, the abbreviation \blue{\textsc{Csp}} refers to \blue{constraint satisfaction problem}.
Given a \textsc{Csp}
\\[0.2cm]
\hspace*{1.3cm}
 $\mathcal{P} = \langle \texttt{Vars}, \texttt{Values}, \texttt{Constraints} \rangle$, 
...
to $\mathcal{P}$ if, under $A$, all constraints are fulfilled, that is:
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{eval}(f, A) = \texttt{true}$ \quad for every $f \in \texttt{Constraints}$.
\\[0.2cm]
Moreover, a \blue{partial variable assignment} $B$ for $\mathcal{P}$ is a function \index{partial variable assignment}
\\[0.2cm]
\hspace*{1.3cm}
$B: \texttt{Vars} \rightarrow \texttt{Values} \cup \{ \Omega \}$, with $\Omega$ symbolizing the undefined value.
\\[0.2cm]
Therefore, a partial variable assignment does not assign values to all variables, but only to a subset of $\texttt{Vars}$. The \blue{domain} $\texttt{dom}(B)$ of a partial variable assignment $B$ is defined as the set of variables assigned a value different from $\Omega$, namely:
\\[0.2cm]
\hspace*{1.3cm}
...
In order to write down the last 5 constraints we implement the auxiliary function $\texttt{allDifferent}(V)$
...
\item Rudolf is desperately begging for euthanasia and his room is between the room of the Camel smoker and the
...
satisfaction problems.  The most important applications seem to be variants of
...
compute a solution.  In contrast, the seven queens problem took about 1.7 seconds.  As we have
\\[0.2cm]
\hspace*{1.3cm}
...
For the $n$ queens problem the number of possible variable assignments grows as fast as $n^n$.
...
\item Once we have picked a variable $\texttt{x}$, we next iterate over all values $\texttt{v}$ in
      ...
      \blue{propagate} the consequences of this assignment: 
      \begin{enumerate}
...
the variable $\texttt{x}$  by propagating the consequences of setting $\texttt{x}$ to $\texttt{v}$.  To this end
...
\begin{Definition}[Consistent Value for a Variable]
A value $v$ is \blue{consistent} for the variable $x$ with respect to the constraint $f$ 
...
the algorithm for \blue{consistency maintenance} is shown below. \index{consistency maintenance}
\begin{enumerate}
...
\textsl{
  ...
  \item My oldest sister has twice as many brothers as she has sisters.
...
In order to infer the number of children we first have to determine whether I am male or female.
If I were female, I would have as many brothers as my sister has.  Now if my sister would have twice as many
brother, this could only be true if I had no brothers.  But then I would not have any sisters either and this
contradicts the fact that I have an oldest sister.  This contradiction shows that I have to be male.
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{export PATH="\symbol{126}/opt/anaconda3/envs/ai/bin/}
\\[0.2cm]
...
\textsl{
  ...
  \end{enumerate}}
  You may assume that the prices of these delicacies are integer valued.
\eox
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-02/Sudoku-Z3-Frame.ipynb}{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/\\
      \hspace*{2.6cm}  
          Python/Chapter-02/Sudoku-Z3-Frame.ipynb}
    \eox
\pagebreak
