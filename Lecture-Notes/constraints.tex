...
\begin{enumerate}[(a)]
\item The initial section introduces the concept of a constraint satisfaction problem. To elucidate this concept, we explore two examples: \blue{map colouring} and the \blue{eight queens puzzle}. Subsequently, we examine various applications of constraint satisfaction problems.
\item The most basic algorithm for addressing a constraint satisfaction problem is \blue{brute force search}. The principle of \emph{brute force search} involves examining every possible \blue{variable assignment}.
\item Often, the search space is so vast that enumerating all variable assignments becomes impractical. \blue{Backtracking search} enhances brute force search by intertwining the generation of variable assignments with the evaluation of constraints, significantly boosting the efficiency of the search.
\item Backtracking search can be further refined through the integration of \blue{constraint propagation} and the application of the \blue{most restricted variable} heuristic.
\item Additionally, verifying the \blue{consistency} of values assigned to different variables can substantially reduce the search space.
...
\\[0.2cm]
The concepts delineated thus far will be explained through two examples.
