...
In a constraint satisfaction problem we have been given a number of \blue{formul\ae} and search for values that can be
assigned to the \blue{variables} occurring in these formul\ae\  so that the formul\ae\  evaluate as true.
...
\myFig{csp-backtrack-solver.stlx-1} shows a simple \textsc{Csp} solver 
...
\end{figure}
\myFig{csp-constraint-propagation.stlx-2} show the implementation of the procedure $\texttt{most\_constrained\_variable}$
...
The function $\texttt{propagate}$ show in \myFig{csp-constraint-propagation.stlx-3} takes the following inputs:
\begin{enumerate}[(a)]
