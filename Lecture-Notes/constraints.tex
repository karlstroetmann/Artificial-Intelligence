...
\item If the assignment is not yet complete, we randomly\footnote{
      ...
      pick a variable $\texttt{var}$ from the set of those
      $\texttt{Variables}$ that still have no value assigned.  
...
$\texttt{f}$ from the set $\texttt{Constraints}$, it checks whether $\texttt{f}$ is satisfied.  This is done
using the auxiliary $\texttt{eval\_constraint}$.  
...
When I tested this brute force search with the eight queens problem, it took less than 300 seconds to
compute a solution.  In contrast, the seven queens problem only took 16 seconds.  As we have
\\[0.2cm]
\hspace*{1.3cm}
$\ds\frac{8^8}{7^7} \approx 20.3$ \quad \and \quad $\ds\frac{684}{16} \approx 18.8$
\\[0.2cm]
...
\item Next, for every constraint $\texttt{f}$ of the given $\texttt{CSP}$, we compute the set of variables that
      ...
      stored as a dictionary, we can only check the constraint $\texttt{f}$ iff all of the variables occurring
      in $\texttt{f}$ are elements of the domain of $\texttt{Assignment}$. 
...
\end{enumerate}
Next, we discuss the implementation of the procedure $\texttt{bt\_search}$.  This procedure gets a partial assignment
...
\item Next, it is tried to assign a $\texttt{value}$ to the selected variable $\texttt{var}$.  After assigning
      ...
      that is caught by one of the \texttt{check}-blocks that are avove the \texttt{backtrack}-statement in the
      call stack.
\end{enumerate}
...
\item The procedure $\texttt{select\_unassigned\_variable}$ takes a partial $\texttt{Assignment}$ and the set
      of all $\texttt{Variables}$.  It randomly selects a variable $\texttt{v}$ from $\texttt{Variables}$ such that 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item The procedure $\texttt{is\_consistent}$ takes a variable $\texttt{var}$, a $\texttt{value}$, a partial 
      ...
      furthermore, have the the property that
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{Vars}(\texttt{Formula}) \subseteq \texttt{dom}(\texttt{NewAssignment})$,
      \\[0.2cm]
      i.e.~all variables occurring in $\texttt{Formula}$ need to have a value assigns in
      $\texttt{NewAssignment}$.  The reasoning is as follows:
      \begin{enumerate}
      ...
      \item If $\texttt{dom}(\texttt{NewAssignment}) \not\subseteq \texttt{Vars}(\texttt{Formula})$, then
            $\texttt{Formula}$ can not be evaluated anyway. 
      \end{enumerate}
      Now if any of the $\texttt{Formula}$ in the $\texttt{Constraints}$ evaluates to $\texttt{false}$, then $\texttt{NewAssignment}$ is not
