...
\item Line 1 and 2 declare the variables \texttt{b} and \texttt{g} as integer variables.
      ...
      With Z3 we are not confined to use a finite set of values.  Instead we can use inter variables and also
      floating point variables.

      The syntax of \texttt{Z3} files is \href{https://en.wikipedia.org/wiki/Lisp_(programming_language)}{lisp}
      like.  As we will only use the \textsl{Python} API of Z3 you do not need to worry about this syntax.
\item Line 4 specifies the equation $b - 1 = g$ as a constraint.

      Note that we can use operators only in prefix notation here.
...
$b = 4$ and $g = 3$ is the solution, i.e.~there are 4 boys and 3 sisters.
...
\noindent
Instead of using the command line to solve \textsc{Csp}s it is more convenient to utilize the \textsl{Python}
interface of Z3.  For example, the Python program shown in Figure \ref{fig:Brothers-and-Sisters.ipynb} solves
the text problem given above via \texttt{Z3}.
\begin{enumerate}
