...
\begin{Definition}[Search Problem]
  ...
  \item $\texttt{nextStates}$ is a function taking a state as input and returning the set of those
        ...
        \\[0.2cm]
        \hspace*{1.3cm}
        $\texttt{nextState}:S \times A \rightarrow 2^S$.
        \\[0.2cm]
  ...
  solution and, furthermore, the length of $p$ is minimal among all other solutions. \eox
\end{Definition}
...
\item Line 1 defines the auxiliary function \texttt{problem}.
      ...
      that same shore, then $\textsl{problem}(m, i)$ is \texttt{true} iff there the missionaries have a problem on that
      shore.  
...
\item Line 5 to 18 define the function \texttt{nextStates}.  A state $s$ is represented as a triple of
      ...
      \end{enumerate}
\item If the boat is on the southern shore instead, then missionaries and infidels will be crossing
      ...
      \texttt{[m - mb, i - ib, 0]}.
      \\[0.2cm]
      As the number of missionaries on the southern shore is $3 - m$ and the number of infidels on the
      southern shore is $3 - m$, \texttt{mb} is now a member of the set $\{0,\cdots,3 -m\}$, while 
      \texttt{ib} is a member of the set $\{0,\cdots,3 - i\}$.
\item Finally the start state and the goal state are defined in line 19 and line 20.
\end{enumerate}
The code in Figure \ref{fig:missionaries.stlx} does not define the state of the search problem.  The
reason is that, in order to solve the problem, we do not need to define this set in order to solve
the problem.  If we want to, we can define the set of states as follows:
\begin{verbatim}
...
Next, we want to develop an algorithm that can solve puzzles of the kind of the missionaries and
cannibals problem automatically.  The easiest algorithm to solve search problems is 
\href{https://en.wikipedia.org/wiki/Breadth-first_search}{breadth first search}. 

\section{The Sliding Puzzle} 
The $3 \times 3$ sliding puzzle is played on a
...
instead.  The $4 \times 4$ sliding puzzle is also known as the 15 puzzle. 
...
that can be positioned in $9!$ different ways.  It turns out that only half the positions are
...
$9! / 2 = 181,440$.
\\[0.2cm]
This is already a big number, but $181440$ states can still be stored in a modern computer.  However, the
...
bytes state.  Hence we would need 
\\[0.2cm]
\hspace*{1.3cm}
...
\item Given a $\mathtt{state}$, the procedure $\mathtt{newStates}$ computes the set of all states
      that can be reached in one step from $\mathtt{state}$.
\end{enumerate}
\pagebreak
...
\item As long as the set $\mathtt{Frontier}$ does not contain the state $\mathtt{goal}$, we extend this set 
      by adding all states that can be reached in step from a state in $\mathtt{Frontier}$.
\end{enumerate}
In order to avoid loops, an implementation of breadth also keeps track of those states that have
...
\\[0.2cm]
Figure \ref{fig:breadth-first-search.stlx} on page \pageref{fig:breadth-first-search.stlx} shows an implementation of
...
inspected all the states.  This is typical behaviour for breadth first search.
...
\item The solution founnd by breadth first search is \emph{\color{blue}optimal}, i.e.~it is the
...
$\mathtt{Visited}$.  In many cases, the state space is so big that this is not possible.
\vspace*{\fill}
\pagebreak
...
interested in the path itself, then depth first search is simply not an option.
...
just fine for small problems like the missionaries and cannibals problem, it does not work at all
for a problem of the size of the $3 \times 3$ sliding puzzle.  The reason is that each recursive
invocation of the function $\mathtt{dfs}$ needs to copy the parameters onto the stack.  However, the
parameters $\mathtt{Parent}$ and $\mathtt{Visited}$ grow linearly with the length of the path that
is explored.  We have already seen that the solution path that is found by depth first search has a
length of more than $9000$.  The resulting overhead is prohibitive.
...
\item When performing  the computation for the limit of $30$, all states that have a distance form start that
      ...
      are only two states that have a distance of $31$ from $\mathtt{start}$.  Hence, the nearly the entire
...
bigger than the running time of the stack base implementation:  On my computer, the recursive implementation
...
\item Otherwise, the procedure $\mathrm{deepeningSearch}$ is called.  This procedure starts and iterative deepening
      ...
      $\mathrm{deepeningSearch}$ returns a pair.  The first component of this pair is the state $s$.  This is
      the state in Distance that has been reached by the $\mathrm{IDA}^*$ search.  The second component is the
      path $P$ that leads from the node $s$ to the node $\mathrm{goal}$.  However, $P$ does not include the node $s$.
      Hence, in order to compute a path from $\mathtt{start}$ to $\mathtt{goal}$, we still have to compute a
      path from $\mathtt{start}$ to $s$.  This path is then combined with the path $P$ and returned.
\end{enumerate}
