...
examples.  We start with the \blue{missionaries and cannibals puzzle}.  Next, we use the 
...
\begin{enumerate}
\item Aaron has 1,000 gold coins.
\item Benjamin has 700 gold coins.
\item Chaim has 300 gold coins.
\end{enumerate}
...
together. The challenge arises if a thief, or a pair of thieves, is left with a quantity of gold greater than
their own, they will abscond with it. 
...
\\[0.2cm]
in the directory
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/1 Search/Three-Thieves.ipynb}{Python/1 Search/Three-Thieves.ipynb}. 
\eox
...
\end{enumerate}
Unfortunately, due to a bug in \textsl{Python} \texttt{3.11}, the \textsl{Python} kernel just dies
...
def dls(state, goal, next_states, Path, PathSet, limit):
    ...
    for ns in next_states(state):
        if ns not in PathSet:
            Result = dls(ns, goal, next_states, Path+[ns], PathSet|{ns}, limit)
            if Result:
                return Result
\end{python3code}
\vspace*{-0.3cm}
...
def search(start, goal, next_states):        
    ...
    while FrontierA and FrontierB:
        for Path in bfs_one_step(FrontierA, ParentA, ParentB, next_states):
            return Path
        for Path in bfs_one_step(FrontierB, ParentB, ParentA, next_states):
            return Path[::-1]
    ...
    for s in Frontier:
        for ns in next_states(s):
            if ns not in ParentA:
                ...
                ParentA[ns]  = s
                if ns in ParentB:
                    yield combinePaths(ns, ParentA, ParentB)
    Frontier.clear()
...
\item The \texttt{for}-loop in line 7 first computes those states that can be reached from $\texttt{FrontierA}$ and have not been
      visited from $\texttt{start}$.  The function
      \texttt{bfs\_os} is a \blue{generator} that tries to compute a path that connects \texttt{start} with
      goal.  If such a path is found, the generator \blue{yields} this path which is then returned in line 8.

      The details of the generator \texttt{bfs\_os} are discussed below.
\item Similarly, the \texttt{for}-loop in line 9 tries to find a path that connects \texttt{goal} with
      \texttt{start} by trying to expand states in \texttt{FrontierB}.
\item The function \texttt{bfs\_os} takes five arguments:
      \begin{enumerate}[(a)]
      ...
      \item \texttt{ParentB} is a dictionary that is similar to \texttt{ParentA} but that instead contains
            ...
            contains the states as keys that have been found while searching from \texttt{goal} and if instead
             ...
             searching from \texttt{goal}.
      \item For every state $s$, we have that $\texttt{next\_states}(s)$ is the set of states that can be
            reached in one step from $s$.
      \end{enumerate}
      The function \texttt{bfs\_os} is a \blue{generator}, i.e.~it does not return a result but rather
      it \blue{yields} a result when we iterate over it.  A result is only yielded when the function finds a
      state \texttt{ns} that is both reachable from the state \texttt{start} and the state \texttt{goal}.

      Otherwise, the function just updates the set \texttt{Frontier} to contain those states that can be
      reached in one step from the previous version of the set \texttt{Frontier}.  Hence, the function
      \texttt{bfs\_os} performs one iteration of breadth first search.
\end{enumerate}
...
\item The Variable \texttt{Visited} collects all states that have been \blue{visited}.  A node $n$ counts as
      \blue{visited} when all of its neighbours have inspected in line 11.
...
\end{enumerate}
Best first search solves the instance of the $3 \times 3$ puzzle shown in \myFig{8-puzzle.pdf} in less than a
...
$4 \times 4$ puzzle shown in \myFig{start-goal.stlx} in less than second.  It visits 8224 different states in
...
by the A\(^*\) search algorithm is consistent, it is assured to find the shortest path. 
...
differ on in line 13 where the priority of a path is computed.  With A$^*$ search, the priority of a path $P$
...
Furthermore, the good news about $\mathrm{A}^*$ search is that, provided the heuristic is consistent, the path which is
...
\begin{Theorem}[Completeness and Optimality of $\mathrm{A}^*$ Search] \hspace*{\fill} \linebreak
  ...
  $h: Q \rightarrow \mathbb{R}$ is a consistent heuristic for $\mathcal{P}$,
...
\begin{minted}[ frame         = lines,
    ...
    def search_os(PQ, Path, goal, VisitedA, VisitedB, next_states, heuristic):
        ...
        if state in VisitedB:
            yield Path[:-1] + VisitedB[state][::-1]
        for ns in next_states(state):           
            if ns not in VisitedA:
                prio = len(Path) + 1 + heuristic(ns, goal)
...
breadth first search subsection.  We can try to do something similar with the $\mathrm{A}^*$ algorithm and develop a
