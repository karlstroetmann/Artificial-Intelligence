...
$s$.  In particular, once the state $y_{m-1}$ is explored, we will have 
\\[0.2cm]
\hspace*{1.3cm}
...
The reason that we will always find a path to the goal, provided such a path exists is the following:
Initially, the priority queue \texttt{Frontier} will just contain the pair $[d, \mathtt{start}]$ where 
\\[0.2cm]
\hspace*{1.3cm}
$d := \mathtt{heuristic}(\mathtt{start}, \mathtt{goal})$.
\\[0.2cm]
Now if at any time the pair removed the priority queue \texttt{Frontier} has the form
\\[0.2cm]
\hspace*{1.3cm}
$[e, \mathtt{state}]$,
\\[0.2cm]
then the pairs that are added to \texttt{Frontier} in this iteration either have the form
\\[0.2cm]
\hspace*{1.3cm}
$[e, \mathtt{ns}]$ \quad,  $[e+1, \mathtt{ns}]$, \quad or \quad $[e+2, \mathtt{ns}]$,
\quad where $\mathtt{ns} \in \texttt{neighbour}(\mathtt{state})$.
\\[0.2cm]
Hence, all the pairs on the priority queue \texttt{Frontier} either have the priority $e$, $e+1$, or $e+2$.
There can be only finitely many states with priority $e$.  Once these have been removed, the
search will carry on to inspect those states with priority $e+1$, then those with priority $e+2$ and so on.
Therefore, eventually all states with a priority less than the priority of the state \texttt{goal} will be
visited.  After that, eventually the \texttt{goal} is reached.
\qed
...
is allowed to visit at most $3\,000$ states, the algorithm takes less than $6$ seconds.
