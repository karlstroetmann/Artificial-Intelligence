...
\item The \texttt{for}-loop in line 7 first computes those states that can be reached from $\texttt{FrontierA}$ and have not been
      ...
      \texttt{bfs\_one\_step} is a \blue{generator} that tries to compute a path that connects \texttt{start} with
      \texttt{goal}.  If such a path is found, the generator \blue{yields} this path which is then returned in
      line 8. 

      The details of the generator \texttt{bfs\_one\_step} are discussed below.
...
\item The function \texttt{bfs\_one\_step} takes five arguments:
      \begin{enumerate}[(a)]
      ...
      \end{enumerate}
      The function \texttt{bfs\_one\_step} is a \blue{generator}, i.e.~it does not return a result but rather
