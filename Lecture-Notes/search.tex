...
def search(start, goal, next_states):        
    ...
    while FrontierA and FrontierB:
        for Path in bfs_os(FrontierA, ParentA, ParentB, next_states, False):
            return Path
        for Path in bfs_os(FrontierB, ParentB, ParentA, next_states, True):
            return Path

def bfs_os(Frontier, ParentA, ParentB, next_states, flip: bool):
    NewFrontier = set()
    for s in Frontier:
        for ns in next_states(s):
