...
\\[0.2cm]
\hspace*{1.3cm}
$[e, \mathtt{ns}]$ \quad or \quad $[e+1, \mathtt{ns}]$,
\quad where $\mathtt{ns} \in \texttt{neighbour}(\mathtt{state})$.
\\[0.2cm]
Hence, all the pairs on the priority queue \texttt{Frontier} either have the priority $e$ or the priority
$e+1$.  There can be only finitely many states with priority $e$.  Once these will have been removed, the
