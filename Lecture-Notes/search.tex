...
\item \textts{nextStates} is a function that takes a state as a parameter.  For a state $s$,
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathtt{nextStates}(s)$
      \\[0.2cm]
...
\item \textts{limit} is the upper bound for the path from \textts{start} to \textts{goal}.  If the
      procedure \textts{dl\_search} is not able to find a path from \textts{start} to \textts{goal} that has
