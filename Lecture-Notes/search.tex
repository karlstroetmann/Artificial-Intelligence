...
\begin{Definition}[Search Problem]
  ...
  \end{enumerate}
  A path $p = [s_1, \cdots, s_n]$ is a \blue{minimal solution} to the search problem $\mathcal{P}$ iff it is a
...
In order to prove the completeness and the optimality of the $\textrm{A}^*$ search algorithm we find convenient to
...
The implementation of $\textrm{A}^*$ search shown in Figure \ref{fig:a-star-search-path.stlx} on page
...
\begin{enumerate}
\item The dictionary $\texttt{Parent}$ that is used in our first implementation of $\textrm{A}^*$ search 
...
order to better understand the $\textrm{A}^*$ algorithm.  The first of these notions is the notion of \blue{explored}
states.  A state is said to have been \blue{explored} iff it has been removed from the priority queue $\texttt{Frontier}$.  
...
\texttt{Explored} serves no purpose in the implementation of $\textrm{A}^*$ search.  It has only been added to aid this
proof.  Furthermore, we define a state as \blue{visited} iff it has been entered into the 
dictionary $\texttt{Distance}$.
...
\\[0.2cm]
From the definition of the dictionary $\texttt{Distance}$ it is obvious that
\\[0.2cm]
\hspace*{1.3cm}
...
found a path from $\texttt{start}$ to $s$ that has the length $\texttt{Distance}[s]$.  
...
\noindent
\textbf{Claim One}: If a state $s \in Q$ has been explored, then we have
\\[0.2cm]
\hspace*{1.3cm}
...
\noindent
\textbf{Proof of Claim One:}  The proof of Claim One is a proof by contradiction.  We assume that $s$ is a
...
f(y_{k+1}) & =  & \texttt{Distance}[y_{k+1}] + \texttt{heuristic}(y_{k+1})     \\[0.2cm]
          & =  & \texttt{Distance}[y_{k+1}] + \texttt{heuristic}(y_{k+1})  \\[0.2cm]
          ...
          & =  & k+1 + m-k-1 + \texttt{heuristic}(s) \\[0.2cm]
          & =  & m + \texttt{heuristic}(s). 
\end{array}
$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$f(y_{k+2}) \leq k+2 + (m-k-2) + \texttt{heuristic}(s)$
\\[0.2cm]
...
\\[0.2cm]
However, $f(s)$ is the priority of the state $s$ in the priority queue, while $f(y_{k+1})$ is the priority
...
$s$.  In particular, once the state $y_{m-1}$ is explored, the estimated total distance of $s$ would be
 $m + \texttt{heuristic}(s)$ contradicting the fact that it is $n + \texttt{heuristic}(s)$. \red{$\lightning$}
\vspace*{0.2cm}

\noindent
Claim One implies that if the $\textrm{A}^*$ algorithm finds a path from $\texttt{start}$ to $\texttt{goal}$, then this
path must be optimal.  This follows from the fact that before we check whether the search has reached the
$\texttt{goal}$, the $\texttt{state}$ that is compared to $\texttt{goal}$ has been explored.  In order to
finish the proof we still need to show that we will find a path provided a path exists.
