...
\begin{Definition}[Search Problem]
  ...
  \item \textts{goal} is the \blue{goal state}, hence $\mathtt{goal} \in Q$.
        ...
        Sometimes, instead of a single $\textts{goal}$ there is a set of goal states $\textts{Goals}$.
  \end{enumerate}
  ...
  is that the path consists on $n-1$ \blue{edges} \index{edge} of the form $\langle s_i, s_{i+1} \rangle$ where
  $i \in \{1, \cdots, n-1\}$.
  ...
  \item $s_n = \mathtt{goal}$, i.e.~the last element of the path is the goal state.
        ...
        If instead of a single $\textts{goal}$ we have a set of $\textts{Goals}$, then the last condition
        is changed into
        \\[0.2cm]
        \hspace*{1.3cm}
        $s_n \in \mathtt{Goals}$.
...
\item Line 1 defines the auxiliary function \textts{problem}.
      ...
      infidels. 
\item Line 3 defines the auxiliary function \textts{no\_problem}.
...
\item Lines 5 to 18 define the function \textts{next\_states}.  A state $s$ is represented as a triple of
      ...
      \item If this is the case,  then the states reachable from the given state $s$ are those
            states where $\textts{mb}$ missionaries and $\textts{ib}$ infidels cross the river.
            After $\textts{mb}$ missionaries and $\textts{ib}$ infidels have crossed the river and
      ...
      \item Since the number $\textts{mb}$ of missionaries leaving the western shore can not be greater
...
\item If the boat is on the eastern shore instead, then the missionaries and the infidels will be crossing
      ...
      eastern shore is $3 - \mathrm{i}$, $\textts{mb}$ has to be a member of the set $\{0,\cdots,3 -\mathtt{m}\}$, while
      $\textts{ib}$ has to be a member of the set $\{0,\cdots,3 - \mathtt{i}\}$.
...
\item \textts{find\_tile} is an auxiliary function that is needed to implement the function \textts{next\_states}.
      It is called with a $\textts{number}$ and a $\textts{State}$ and
      returns the row and column where the tile labelled with $\textts{number}$ can be found.
\item \textts{moveDir} takes a $\textts{State}$, the $\textts{row}$ and the $\textts{col}$umn
      where to find the empty square and a direction in which the empty square should be moved.
      This direction is specified via the two variables $\textts{dx}$ and $\textts{dy}$.  The tile
...
\item Given a $\textts{State}$, the function \textts{next\_states} computes the set of all states
      that can be reached in one step from $\textts{State}$.  The basic idea is to find the position of the
...
\item Given a search problems $\langle Q,\;\mathtt{next\_states},\; \mathtt{start},\; \mathtt{goal}\rangle$,
      we initialize a set $\textts{Frontier}$ to contain the state $\textts{start}$.

      In general, $\textts{Frontier}$ contains those states that have just been discovered and whose successors have not
      yet been seen.
\item As long as the set $\textts{Frontier}$ does not contain the state $\textts{goal}$, we recompute this set
      by adding all states to it that can be reached in one step from a state in $\textts{Frontier}$.
      Then, the states that had been previously present in $\textts{Frontier}$ are removed.
      These old states are then added to the set $\textts{Visited}$.
\end{enumerate}
In order to avoid loops, an implementation of breadth first search keeps track of those states that have
been visited in the set $\textts{Visited}$.  Once a state has been added to
the set $\textts{Visited}$,  it will never be revisited again.
Furthermore, in order to keep track of the path leading to the goal, we utilize a dictionary called
$\textts{Parent}$.  For every state $s$ that is in $\textts{Frontier}$, $\mathtt{Parent}[s]$ is the state that
caused $s$ to be added to the set $\textts{Frontier}$, i.e.~for all states $s\in\mathtt{Frontier}$ 
we have 
\\[0.2cm]
\hspace*{1.3cm}
...
breadth first search in \textsl{Python}.  The function \textts{search} takes three arguments to solve a
\blue{search problem}: 
\begin{enumerate}[(a)]
\item \textts{start} is the \blue{start state} of the search problem,
\item \textts{goal} is the \blue{goal state} of the search problem, and
\item \textts{next\_states} is a function with signature      
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{next\_states}:Q \rightarrow 2^Q$, 
      \\[0.2cm]
...
\end{enumerate}
If successful, \textts{search} returns a path from \textts{start} to \textts{goal} that is a solution of the
search problem 
\\[0.2cm]
\hspace*{1.3cm}
$\langle Q, \texttt{next\_states}, \texttt{start}, \texttt{goal} \rangle$.
\\[0.2cm]
Next, we discuss the implementation of the function \textts{search}:
\begin{enumerate}
\item $\textts{Frontier}$ is the set of all those states that have been encountered but whose
      neighbours have not yet been explored.  Initially, it contains the state $\textts{start}$.
\item $\textts{Visited}$ is the set of all those states, all of whose neighbours have already been
      added to the set $\textts{Frontier}$ in the last iteration of the \texttt{while} loop.  In order to avoid
      infinite loops, these states must not be visited again.
\item $\textts{Parent}$ is a dictionary keeping track of the predecessors of the state that have been reached.
      The only state with no real predecessor is the state \texttt{start}.  By convention, \texttt{start} is its
      own predecessor.
\item As long as the set $\textts{Frontier}$ is not empty, we add all neighbours of states in
      $\textts{Frontier}$ that have not yet been visited to the set $\textts{NewFrontier}$.
      When doing this, we keep track of the path leading to a new state $\textts{ns}$ by storing its
      parent in the dictionary $\textts{Parent}$.
\item If the new state happens to be the state $\textts{goal}$, we return a path leading from
      $\textts{start}$ to $\textts{goal}$.  The function $\textts{pathTo}()$ is shown in Figure
      \ref{fig:pathTo.stlx} on page \pageref{fig:pathTo.stlx}.
\item After we have collected all successors of states in $\textts{Frontier}$, the states
      in the set $\textts{Frontier}$ have been visited and are therefore added to the set
      $\textts{Visited}$, while the set $\textts{Frontier}$ is updated to $\textts{NewFrontier}$.
\end{enumerate}
...
\end{minted}
\vspace*{-0.3cm}
\caption{The function $\textts{pathTo}$.}
\label{fig:pathTo.stlx}
\end{figure}
The function call $\mathtt{pathTo}(\mathtt{state}, \mathtt{Parent})$ constructs a path reaching
from $\textts{start}$ to $\textts{state}$ in reverse by looking up the parent states.  It uses the fact that
...
}, while 181,439 states are touched.  The
Again, we see that breadth first search touches nearly all the states reachable from the start state.
If we measure the memory consumption, we discover that the program uses nearly 90 megabytes of memory.
...
the set $\textts{Frontier}$ contains exactly those states that have a distance of $n$ to the state
$\textts{start}$.  
\begin{description}
\item[Base Case:] $n = 0$.

      After $0$ iterations of the \texttt{while} loop, i.e.~before the first iteration of this loop,
      the set $\textts{Frontier}$ only contains state $\textts{start}$.  As this is the only state that has a
      distance of $0$ to the state \textts{start}, the claim is true in this case.
\item[Induction Step:] $n \mapsto n+1$.
      ...
      states that can be reached in one step from a state in $\textts{Frontier}$ are added to the new $\textts{Frontier}$,
      provided there is no shorter path to them.  By induction hypothesis, there is a shorter path to a state
      if this state is already a member of the set $\textts{Visited}$.  In this case, the state would not be
      added to \textts{NewFrontier}.  Otherwise, the shortest path to a state that is
      reached in iteration $n+1$ has the length $n+1$ and the state is added to \textts{NewFrontier}.  Hence,
      the claim is true after $n+1$ iterations also.
\end{description}
Now, if there is a path from $\textts{start}$ to $\textts{goal}$, there must also be a shortest
path.  Assume this path has a length of $k$.  Then, $\textts{goal}$ is reached in the $k^{\textrm{th}}$
...
many problems:  If the $\textts{goal}$ is far from the $\textts{start}$, breadth first search will use
a lot of memory because it will store a large part of the state space in the set
$\textts{Visited}$.  In many cases, the state space is so big that this is not possible.  For example, it is
...
uses a queue to store the set \textts{Frontier}.   Here we use the module \textts{deque} from the package
\textts{collections}. This module implements a
...
\begin{enumerate}
\item Line 4 initializes the $\textts{Frontier}$ as a double-ended queue that contains the state \texttt{start}.
\item In line 7 we remove the oldest element in the queue $\textts{Frontier}$, which is supposed to be at the
...
\end{enumerate}
Additionally, we have used the fact that the information contained in the set $\textts{Visited}$ is already
available in the dictionary $\textts{Parent}$, because we when we visit a state $s$, we add an entry for
...
\begin{enumerate}
\item In order to save memory, \ac{dfs} removes the set \textts{Visited} of \ac{bfs}.
\item While \ac{bfs} ensures that every state is visited by implementing the \textts{Frontier} as a queue,
      \ac{dfs} replaces this queue by a stack.  This way, \ac{dfs} tries to get as far away from the state
      \textts{start} as early as possible.  In order to prevent loops, we still have the parent dictionary.
...
$\textts{goal}$ that is far away from $\textts{start}$ without exploring the whole state space.
...
\end{figure}
The implementation of  \textts{search} works as follows:
\begin{enumerate}
\item Any states that are encountered during the search are placed on top of the stack \textts{Stack}.
\item In order to record the information how a state has been added to the \textts{Stack}, we have a dictionary
      \textts{Parent}.  For every state $s$ that is on \textts{Stack}, $\texttt{Parent}[s]$ returns a state $p$
      such that $s \in \texttt{next\_states}(p)$,  i.e. $p$ is the state that immediately precedes $s$ on the
      path that leads from \textts{start} to $s$.  
\item Initially, \textts{Stack} only contains the state \textts{start}.
\item As long as \textts{Stack} is not empty, the \textts{state} on top of \textts{Stack} is replaced by all
      states that be reached in one step from \textts{state}.  However, in order to prevent depth first search
      to run in circles, only those states \textts{ns} from the set \texttt{next\_states(state)} are appended to
      \textts{Stack} that have not been encountered previously.  This is checked by testing 
      whether \textts{ns} is in the domain of \textts{Parent}.
\item When the \textts{goal} is reached,  a path leading from \textts{start} to \textts{goal} is returned.
\end{enumerate}
...
The memory consumption is reduced to 14 megabytes.  This is still a lot and is due to the fact that we still
have to maintain the set $\textts{Parent}$.  Fortunately, we will 
be able to get rid of the set $\textts{Parent}$ when we develop a recursive implementation of depth first
...
shortest path from $\textts{start}$ to $\textts{goal}$ has only 31 steps, the solution found by depth
...
longer need the dictionary $\textts{Parent}$ to record the parent of each node.  The resulting
...
\end{figure}
The only purpose of the function \textts{search} is to call the function $\textts{dfs}$, which needs two
additional arguments.  These arguments are called $\textts{Path}$ and $\textts{PathSet}$.  The idea is that $\textts{Path}$ is
a path leading from the state $\textts{start}$ to the current $\textts{state}$ that is the first
argument of the function $\textts{dfs}$, while $\textts{PathSet}$ is a set containing all the elements of the
list $\textts{Path}$.  The argument $\textts{PathSet}$ is only used for efficiency reasons:  In order to avoid
infinite loops, when we discover a node we have to check that this node does not occur already in $\textts{Path}$.
However, checking whether an element occurs in the list $\textts{Path}$ is much slower than checking whether
the element occurs in the corresponding set $\textts{PathSet}$.  On the first invocation of $\textts{dfs}$, the
parameter $\textts{state}$ is equal to $\textts{start}$ and therefore $\textts{Path}$ is initialized
as the list containing only $\textts{start}$.

The implementation of $\textts{dfs}$ works as follows:
\begin{enumerate}
\item If $\textts{state}$ is equal to $\textts{goal}$, our search is successful. Since by assumption
      the list $\textts{Path}$ is a path connecting $\textts{start}$ and $\textts{state}$ and we
      have checked that $\textts{state}$ is equal to $\textts{goal}$, we can return $\textts{Path}$ as our solution.
\item Otherwise, $\texttt{next\_states}(\mathtt{state})$ is the set of states that are reachable from $\textts{state}$
      in one step.  Any of the states $\textts{ns}$ in this set could be the next state on a path
      that leads to $\textts{goal}$.  Therefore, we try recursively to reach $\textts{goal}$ from
      every state $\textts{ns}$.  Note that we have to change $\textts{Path}$ to the list
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{Path + [ns]}
      \\[0.2cm]
      when we call the function $\textts{dfs}$ recursively.  This way, we retain the invariant of
      $\textts{dfs}$ that the list $\textts{Path}$ is a path connecting $\textts{start}$ with $\textts{state}$.
\item We still have to avoid running in circles.  In the recursive version of depth first search,
      this is achieved by checking that the state $\textts{ns}$ is not already a member of the list $\textts{Path}$.  In the
      non-recursive version of depth first search, we had used the set $\textts{Parent}$ instead.
      The current implementation no longer has a need for the dictionary $\textts{Parent}$.  This is very
      fortunate since it reduces the memory requirements of depth first search considerably.
\item If one of the recursive calls of $\textts{dfs}$ returns a list, this list is a solution to our
      search problem and hence it is returned.  However, if instead 
      $\textts{None}$ is returned, the \texttt{for} loop needs to carry on and test the other
      successors of $\textts{state}$.
\item If the search was unsuccessful, we have to remove the state $\textts{ns}$ from the $\textts{PathSet}$
      in order to maintain the invariant that $\textts{PathSet}$ is the set of all elements in the list
      $\textts{Path}$.
\item Note that the recursive invocation of $\textts{dfs}$ returns $\mathtt{None}$ if the end of the
...
\begin{enumerate}
\item The function $\textts{search}$ initializes the variable $\textts{limit}$ to 1 and tries to find a solution
      to the search problem that has a length that is less than or equal to $\textts{limit}$.  If a solution is
      found, it is returned.  Otherwise, the variable $\textts{limit}$ is incremented by one and a
...
\item The function $\textts{dls}$ implements a recursive version of depth first search but takes care to compute only
      those paths that have a length of at most $\textts{limit}$.  The name $\textts{dls}$ is short for 
      \blue{\underline{d}epth \underline{l}imited \underline{s}earch}.  If the $\textts{Path}$ has reached a length
      of $\textts{limit}$ but does not end in $\textts{goal}$, the function returns \texttt{None} instead of
      trying to extend this $\textts{Path}$.  Otherwise, the implementation is the same as the recursive
      implementation of depth first search that was shown in Figure \myFig{Depth-First-Search.ipynb}
...
memory.  The reason is that the $\textts{Path}$ can never have a size that is longer than $\textts{limit}$.
However, when we run this program to solve the $3 \times 3$ sliding puzzle, the algorithm takes
about 15 minutes.  There are two reasons for this:
\begin{enumerate}
...
\item Given a state $s$ that is reachable from the $\textts{start}$, there often are a huge number of
      different paths that lead from $\textts{start}$ to $s$.  The version of iterative deepening presented in
      this section tries all of these paths and hence needs a large amount of time.
\end{enumerate}
To check what is really going on, we can change the initial value of $\textts{limit}$ that is set to $1$ in
...
$32$, then the program is able to find the solution in about 4 seconds.  The reason is that in the case that 
$\textts{limit}$ has the value $31$, the program has to check all possible lists $\textts{Path}$ that have a
length of at most $31$.  However, if $\textts{limit}$ has the value $32$, it is sufficient to find any
$\textts{Path}$ of length $32$ that leads to the $\textts{goal}$ and if that $\textts{Path}$ has been found, the
...
\\[0.2cm]
Furthermore, the states $\textts{start}$ and $\textts{goal}$ are defined as
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{start} := \pair(0,0)$ \quad and \quad $\mathtt{goal} := \pair(n,n)$ where $n \in \mathbb{N}$.
\\[0.2cm]
Next, the function $\textts{next\_states}$ is defined as
\\[0.2cm]
\hspace*{1.3cm}
...
\ref{fig:Iterative-Deepening.ipynb} does not terminate.  The reason is that the function $\textts{dls}$ does not
...
from $\textts{start}$ to $\textts{goal}$ is $d$, then all states that have a distance of at most $d$ will be
...
size.  One simple idea is to start searching both from the node $\textts{start}$ and the node $\textts{goal}$
simultaneously.  This approach is known as \blue{bidirectional search}.  \index{bidirectional search}
The justification is that the path starting from $\textts{start}$ and the
path starting from $\textts{goal}$ will meet in the middle and hence they will both have a size of approximately
...
$b = 2$ and that the length of the shortest path leading from $\textts{start}$ to $\textts{goal}$
...
$\textts{Visited}$ in the implementation of \ac{bfs} shown in Figure \ref{fig:Breadth-First-Search.ipynb} is
also available in the dictionary $\textts{Parent}$, we have removed the variable $\textts{Parent}$ in our
implementation of bidirectional breadth first search.

Let us discuss the details of the implementation.
\begin{enumerate}
\item The variable $\textts{FrontierA}$ is the frontier that starts from the state $\textts{start}$, while
      $\textts{FrontierB}$ is the frontier that starts from the state $\textts{goal}$.
\item For every state $s$ that is in $\textts{FrontierA}$, $\mathtt{ParentA}[s]$ is the state that caused $s$
      to be added to the set $\textts{FrontierA}$.  Similarly, for every state $s$ that is in $\textts{FrontierB}$,
      $\mathtt{ParentB}[s]$ is the state that caused $s$ to be added to the set $\textts{FrontierB}$.
\item The bidirectional search keeps running for as long as both sets $\textts{FrontierA}$ and
      $\textts{FrontierB}$ are non-empty and a path has not yet been found.
\item The \texttt{while} loop computes those states that can be reached from $\textts{FrontierA}$ and have not been
      visited from $\textts{start}$.  If a state $\textts{ns}$ is a neighbour of a state $\textts{s}$ from the set
      $\textts{FrontierA}$ and the state $\textts{ns}$ has already been encountered during the search that started
      from $\textts{goal}$, then a path leading from $\textts{start}$ to $\textts{goal}$ has been found and this path
      is returned.  The function \textts{combinePaths} that computes this path by combining the path that leads
      from $\textts{start}$ to $\textts{ns}$ and then from $\textts{ns}$ to $\textts{goal}$ to is shown in Figure
      \ref{fig:combine-paths.stlx} on page \pageref{fig:combine-paths.stlx}.
\item Next, the same computation is done with the role of the states $\textts{start}$ and $\textts{goal}$ exchanged.
\end{enumerate}
...
$4 \times 4$ sliding puzzle since the portion of the search space that needs to be computed is just too big to
fit into memory. 
...
\\[0.2cm]
Now if our heuristic $h$ assumes that the distance of $s_2$ from the $\textts{goal}$ is $h(s_2)$, then the distance of
$s_1$ from the $\textts{goal}$ can be at most $1 + h(s_2)$ because starting from $s_1$ we can first go to $s_2$
in one step and then from $s_2$ to $\textts{goal}$ in $h(s_2)$ steps for a total of $1 + h(s_2)$ steps.  Of
course, it is possible that there exists a shorter path from $s_1$ leading to the $\textts{goal}$ than the one
...
path $p$ from $s$ to the $\textts{goal}$.  Assume this path has the form
\\[0.2cm]
\hspace*{1.3cm}
...
\item The next heuristic is the \blue{number of misplaced tiles} heuristic.  For a state $s$,
      ...
      in the same position as the corresponding tile in $\textts{goal}$.  For example, in \myFig{8-puzzle.pdf}
      ...
      $\textts{goal}$ has no misplaced tiles, hence its heuristic is $0$.  Second, in every step of the sliding
...
\item The \blue{Manhattan heuristic} improves on the previous heuristic.  For two points
      ...
      $\textts{goal}$, we first define the function $\mathtt{pos}(t, s)$ for all tiles
      ...
      states $\textts{stateA}$ and $\textts{stateB}$ and computes the Manhattan distance between these states.
      \begin{enumerate}
      \item First, the size $\textts{n}$ of the puzzle is computed by checking the number of rows of
            $\textts{stateA}$.
      \item Next, the \texttt{for} loops iterates over all rows and columns of $\textts{stateA}$ that do not
            contain a blank tile.  Remember that the blank tile is coded using the number $0$.  The tile at
            position $\pair(\mathtt{rowA}, \mathtt{colA})$ in $\textts{stateA}$ is computed using the expression \texttt{stateA[rowA][colA]} and the
            corresponding position $\pair(\mathtt{rowB}, \mathtt{colB})$ of this tile in state $\textts{stateB}$ is computed using the function
            \textts{find\_tile}.
      \item Finally, the Manhattan distance between the two positions $\pair(\mathtt{rowA}, \mathtt{colA})$ and
            $\pair(\mathtt{rowB}, \mathtt{colB})$ is added to the $\textts{result}$.
      \end{enumerate}
...
ordered with respect to the estimated distance of the state at the end of the path from the $\textts{goal}$.
...
We need only two functions from the module \textts{heapq}:
\begin{enumerate}
...
The function $\textts{search}$ shown in \myFig{Best-First-Search.ipynb} takes four parameters.  The
first three of these parameters are the same as in the previous search algorithms.  The last parameter
$\textts{heuristic}$ is a function that takes two states and then estimates the distance between these states.
Later, we will use the Manhattan distance to serve as the parameter $\textts{heuristic}$.  The details of the
...
\item The variable \textts{PrioQueue} serves as a priority queue.  This priority queue is initialized as a
      ...
      \\[0.2cm]
      where $\textts{Path}$ is a list of states starting from the node \textts{start}.  If the last node on
      this list is called $\textts{state}$, then we have
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathtt{estimate} = \mathtt{heuristic}(\mathtt{state}, \mathtt{goal})$,
      \\[0.2cm] 
      i.e.~\textts{estimate} is the estimated distance between this \textts{state} and \textts{goal} and hence
...
\item As long as \textts{PrioQueue} is not empty, we take the \textts{Path} from the beginning of this
      ...
      \textts{PrioQueue} and, furthermore, this pair is removed from \textts{PrioQueue}.
...
7 milliseconds and uses 100 kilobytes to do so.  However, the solution that is found takes 75 steps.  While
...
estimate of the distance from $s$ to $\textts{goal}$.  Hence, $f(s)$ is the estimate of the \blue{total distance} that
a path connecting $\textts{start}$ and $\textts{goal}$ via the state $s$ would use.
The details of the $\mathrm{A}^*$ algorithm are given in
\myFig{A-Star-Search.ipynb} and are discussed below. 
...
\begin{enumerate}
\item \textts{start} is a state.  This state represents the \blue{start} state of the search problem.
\item \textts{goal} is the \blue{goal} state.
...
\item \textts{Distance} is a dictionary.  For every state $s$ that has been encountered so far,  this
      ...
      Initially, \textts{Distance} is only defined for the state $\textts{start}$.
\item \textts{Estimate} is a dictionary.  For every state $s$ encountered in the search, $\mathtt{Estimate}[s]$
      is an estimate of the length that a path from $\textts{start}$ to $\textts{goal}$ would have if it would
...
\item \textts{Frontier} is a \href{https://en.wikipedia.org/wiki/Priority_queue}{priority queue}.
      The elements of $\textts{Frontier}$ are pairs of the form
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      estimated that a path leading from $\textts{start}$ to $\textts{goal}$ and passing through $s$ would have
      a length of $d$.
      ...
      \item \texttt{S.isEmpty()} checks whether the priority queue \textts{S} is empty.
      \item \texttt{S.member(x)} checks whether \textts{x} is an element of the priority queue \textts{S}.
      \item \texttt{S.insert(x)} inserts \textts{x} into the priority queue \textts{S}.
            This does not return a new priority queue but rather modifies the priority queue \textts{S}.
      \item \texttt{S.delete(x)} deletes \textts{x} from the priority queue \textts{S}.
            This does not return a new priority queue but rather modifies the priority queue \textts{S}.
      \item \texttt{S.pop()} returns the smallest element of the priority queue \textts{S}.
            Furthermore, this element is removed from \textts{S}.
      \end{enumerate}
...
\item Now if this \textts{state} is the \textts{goal}, then a solution has been found.  The path
      corresponding to this solution is computed by the function $\textts{path\_to}$, which utilizes 
...
\item Otherwise, we retrieve the length of the path leading from \textts{start} to \textts{state} from the
      ...
      prove that \textts{stateDist} is not just the length of some path connecting
...
However, the comparison is a bit unfair, because the module \textts{heapq} that is used in best first search
has been implemented  in \texttt{C}, while the module \textts{Set} is implemented in \textsl{Python}.
...
reformulate the algorithm:  In particular, we replace the dictionary $\textts{Parent}$ by a dictionary
$\textts{PathDict}$. For every state $s$ that is reached during $\textrm{A}^*$ search, $\texttt{PathDict}[s]$ returns a
path that connects the node $\textts{start}$ with the node $s$.  Furthermore, we have added a set
$\textts{Explored}$ to the implementation.  This set is only needed for the proof of the optimality of
...
both \blue{complete} and \blue{optimal}, i.e.~if there is a path from \textts{start} to \textts{goal}, then the
...
\begin{enumerate}[(a)]
\item A state has been \blue{visited} iff it has been entered into the dictionary $\textts{Distance}$, i.e.~a
...
\end{enumerate}
To emphasize the notion of explored states I have added the set $\textts{Explored}$ in the implementation of $\textrm{A}^*$ search that
is shown in Figure \ref{fig:A-Star-Search-Path.ipynb} on page \pageref{fig:A-Star-Search-Path.ipynb}.  
Note that the variable $\textts{Explored}$ serves no purpose in the implementation of $\textrm{A}^*$ search:
...
$P := \texttt{PathDict}[s]$, then $P$ is a path from $\textts{start}$ to $s$ and the length of
...
\begin{enumerate}
\item[BC:] The first path that is entered into the dictionary $\textts{PathDict}$ is the path
           $[\texttt{start}]$.  This path connects the node $\textts{start}$ with the node $\textts{start}$.
           Obviously, this path has the length $0$ and that is exactly the value that is entered in the
           dictionary $\textts{Distance}$ in line 3.
\item[IS:] If we add in line 20 the path
           \\[0.2cm]
           \hspace*{1.3cm}
           $\texttt{PathDict}[\texttt{state}] + [\texttt{ns}]$
           \\[0.2cm]
           as a path leading to the state $\textts{ns}$, then by our induction hypotheses we know that
           \\[0.2cm]
           \hspace*{1.3cm}
           $P := \texttt{PathDict}[\texttt{state}]$ 
           \\[0.2cm]
           is a path leading from $\textts{start}$ to $\textts{state}$ and
           that, furthermore, the length of the path $P$ is equal to $\texttt{Distance}[\texttt{state}]$.  When
           we append the state $\textts{ns}$ to this path $P$, the length of the resulting path is
           bigger by one than the length of $P$ and it is obvious that this new path 
           \\[0.2cm]
           \hspace*{1.3cm}
           $P + [\texttt{ns}]$
           \\[0.2cm]
           connects $\textts{start}$ with $\textts{ns}$.  The induction hypotheses tell us that
           $\texttt{Distance}[\texttt{state}]$ is equal to the length of $P$.  Therefore, the length of the new
           path is $\texttt{Distance}[\texttt{state}] + 1$ and since $\textts{stateDist}$ is defined as
...
distance of $s$ from the state \textts{start}, i.e.~we have
\\[0.2cm]
\hspace*{1.3cm}
...
$\textts{start}$ to $s$.  From the definition of the dictionary $\textts{Distance}$ and Claim One it is obvious that
\\[0.2cm]
\hspace*{1.3cm}
$g(s) \leq \texttt{Distance}[s]$.
\\[0.2cm]
The reason is that every time an entry for a state $s$ is added to the dictionary $\textts{Distance}$, we have
found a path from $\textts{start}$ to $s$ that has the length $\texttt{Distance}[s]$.  This might not be the
...
estimated length of a path starting in $\textts{start}$ and ending in $\textts{goal}$ that visits the state $s$ in
between.  Formally, $f(s)$ is defined as follows:
\\[0.2cm]
\hspace*{1.3cm}
$f(s) := \texttt{Distance}[s] + \texttt{heuristic}(s, \mathtt{goal})$.
\\[0.2cm]
Here, $\texttt{Distance}[s]$ is the number of steps that it takes to reach the state $s$ from $\textts{start}$
when following the path stored in $\mathtt{PathDict}[s]$,
while $\texttt{heuristic}(s, \mathtt{goal})$ is the estimated distance of a path from $s$ to $\textts{goal}$.  Note that
the priority queue $\textts{Frontier}$ is ordered according to the estimated total distance.
...
\\[0.2cm]
i.e.~the path leading from $\textts{start}$ to $s$ is guaranteed to be a shortest path.
\vspace*{0.2cm}
...
\\[0.2cm]
i.e.~we assume that $P_1 := \texttt{PathDict}[s]$ is not a shortest path from $\textts{start}$ to $s$.  Then,
there must be another path $P_2$ from \textts{start} to $s$ that is shorter than $P_1$.  Both $P_1$ and $P_2$
start at the same state $\textts{start}$.  Hence, these two paths must have the form
\\[0.2cm]
\hspace*{1.3cm}
...
the first time. The
index $k$ could be $0$.  In that case the two paths would already diverge at the state \textts{start}, but in
...
\\[0.2cm]
Next, as we have $y_{m-1} \in \texttt{next\_states}(y_{m-2})$ the consistency of $\textts{heuristic}$ implies
\\[0.2cm]
\hspace*{1.3cm}
...
attached to the state $y_{k+1}$.  At the latest,  the state $y_{k+1}$ is put onto the priority queue $\textts{Frontier}$
...
\noindent
Claim Two implies that if the $\textrm{A}^*$ algorithm finds a path from $\textts{start}$ to $\textts{goal}$, then this
path must be optimal.  This follows from the fact that before we check whether the search has reached the
$\textts{goal}$, the $\textts{state}$ that is compared to $\textts{goal}$ has been explored and, according to
Claim Two, must therefore have $\mathtt{Distance}[\mathtt{goal}] = g(\mathtt{goal})$.  As $g(\mathtt{goal})$ is
the true distance of the state $\textts{goal}$ from the state $\textts{start}$, this implies that the path that
...
connecting $\textts{start}$ and $\textts{goal}$, then we will find a path.  Therefore, let us assume that there
is a path $P$ connecting $\textts{start}$ and $\textts{goal}$ and that, furthermore, this path is shorter than
...
from $\textts{start}$ that is at most $n$ and this latter set is obviously finite.  Therefore, unless the
$\textts{goal}$ is reached before, all the states on the path $P$ will eventually be explored and this implies that
the $\textts{goal}$ is eventually found. 
\qed
...
The only new idea is that we alternate between the $\mathrm{A}^*$ search starting from $\textts{start}$ and the
$\mathrm{A}^*$ search starting from $\textts{goal}$ depending on the estimated total distance:
\begin{enumerate}[(a)]
\item As long as the search starting from $\textts{start}$ is more promising, we remove states from
      \textts{FrontierA}.
\item Once the total estimated distance of a path starting from $\textts{goal}$ is less than the best total
      estimated distance of a path starting from $\textts{start}$, we switch and remove states from $\textts{FrontierB}$.
\end{enumerate}
...
\textts{start} and \textts{goal} has a length of 36 steps, this shows that bidirectional $\mathrm{A}^*$ search
...
\end{figure}
\begin{enumerate}
\item As in the $\mathrm{A}^*$ search algorithm, the function \textts{search} takes four parameters.
      \begin{enumerate}
      ...
      \item \textts{heuristic} is a function that takes two parameters $s_1$ and $s_2$, where $s_1$ and $s_2$
            ...
            estimate is optimistic, i.e.~the \textts{heuristic} has to be \blue{admissible}.

            Note that this is different from A$^*$ search. A$^*$ search requires that the \textts{heuristic} is 
            \blue{consistent}.
     \end{enumerate}
...
\item \textts{limit} is the upper bound for the length of the path from \textts{start} to \textts{goal}.  If the
...
\item \textts{heuristic} is a function taking two states as arguments.  The invocation
      $\textts{heuristic}(s_1, s_2)$ computes an \blue{estimate} of the distance between the states $s_1$ and $s_2$.  It is
...
idea is to run an $\mathrm{A}^*$ search from the $\textts{start}$ node until memory is more or less
exhausted.  Then, we start $\mathrm{IDA}^*$ from the $\textts{goal}$ node and search until we find any of the
nodes discovered by the $\mathrm{A}^*$ search that had been started from the $\textts{start}$ node.
\index{A$^*$-IDA$^*$ search}
...
and \myFig{A-Star-Ida-Star.ipynb-2}.  We begin with a discussion of the function $\textts{search}$.
\begin{enumerate}
\item The function $\textts{search}$ takes 5 arguments.
      \begin{enumerate}[(a)]
      \item $\textts{start}$ and $\textts{goal}$ are states.  The function tries to find a path connecting
            $\textts{start}$ and $\textts{goal}$.
      \item $\textts{next\_states}$ is a function that takes a state $s$ as input and computes the set of states that are
            reachable from $s$ in one step.
      \item $\textts{heuristic}$ computes an \blue{estimate} of the distance between $s_1$ and $s_2$.  It is
            assumed that this estimate is both \blue{admissible} and \blue{consistent}.
      \item $\textts{size}$ is the maximal number of states that the $\mathrm{A}^*$ search is allowed to
...
\item The basic idea behind the $\mathrm{A}^*$-$\mathrm{IDA}^*$ algorithm is to first use $\mathrm{A}^*$ search
      to find a path from $\textts{start}$ to $\textts{goal}$.  If this is successfully done without visiting
      more than $\textts{size}$ nodes, the algorithm terminates and returns the path that has been found.
      Otherwise, the algorithm switches over to an $\mathrm{IDA}^*$ search that starts from $\textts{goal}$ and
      tries to connect goal to any of the nodes that have been encountered during the $\mathrm{A}^*$ search.
      To this end, the function $\textts{search}$ maintains the following variables.
      \begin{enumerate}
      \item $\textts{Parent}$ is a dictionary associating a parent state with those states that have already been
            encountered during the search, i.e.~we have
            \\[0.2cm]
            ...
            \\[0.2cm]
            Once the goal has been found, this dictionary is used to compute the path from $\textts{start}$ to
            $\textts{goal}$.
      \item $\textts{Distance}$ is a dictionary that remembers for every state $s$ that is encountered during the
            $\mathrm{A}^*$ search the length of the shortest path from $\textts{start}$ to $s$.
      \item $\textts{Estimate}$ is a dictionary.  For every state $s$ encountered in the $\mathrm{A}^*$ search, $\texttt{Estimate}[s]$
            is an estimate of the length that a path from $\textts{start}$ to $\textts{goal}$ would have if it would
            pass through the state $s$.  This estimate is calculated using the equation
            \\[0.2cm]
            \hspace*{1.3cm}
...
\item The $\mathrm{A}^*$ search runs exactly as discussed in section \ref{sec:a-star-search}.  The only difference is that the
      \texttt{while} loop is terminated once the dictionary $\textts{Distance}$ has more than $\textts{size}$
      entries.  If we are lucky, the $\mathrm{A}^*$ search is already able to find the $\textts{goal}$ and the
      algorithm terminates.
\item Otherwise, the function $\textts{id\_search}$ is called.  This function starts an iterative deepening
  $\mathrm{A}^*$ search from the node $\textts{goal}$.  This works as described in section \ref{sec:ida-star-search}.
      ...
      the function $\textts{id\_search}$ via the dictionary $\textts{Distance}$.  The function
      $\textts{id\_search}$ returns a path leading from $\textts{goal}$ to a state $s$ that is a key of the
      dictionary $\textts{Distance}$ and hence has been reached by the  A$^*$ search. 
      In order to compute a path from $\textts{start}$ to $\textts{goal}$, we still have to compute a
      path from $\textts{start}$ to $s$.  This path is then combined with the path $\textts{P}$ and the resulting path
      is returned.

      The expression \texttt{Path[::-1]} in line 24 reverses the list $\textts{Path}$ and 
      hence \texttt{Path[::-1]} is a path leading from the state $s$ to the state $\textts{goal}$.
...
\end{figure}
 ...
 15-puzzle, where the states $\textts{Start}$ and $\textts{Goal}$ are given as follows:
 \begin{verbatim}
        Start :=  +---+---+---+---+    Goal :=  +---+---+---+---+
                  |   |15 |14 | 8 |             |   | 1 | 2 | 3 |
                  +---+---+---+---+             +---+---+---+---+
