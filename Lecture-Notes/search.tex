...
\begin{Theorem}[Completeness and Optimality of $A^*$ Search] 
Assume 
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{P} = \langle Q,\mathtt{nextStates}, \mathtt{start}, \mathtt{goal}\rangle$
...
Assume that the search problem at hand has the \blue{branching factor} $b$, i.e.~for every state $s$ there are
...
are inserted into the priority queue \texttt{Frontier}.  Let us define the \blue{priority distance} of a state
