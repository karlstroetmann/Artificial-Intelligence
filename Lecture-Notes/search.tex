...
the path found by $\mathrm{A}^*$ search.
...
$\mathrm{A}^*$ search algorithm is complete and, furthermore, the solution that is computed is a shortest path
...
\item[BC:] The first path that is entered into the dictionary $\texttt{PathDict}$ is the path
           ...
           dictionary $\texttt{PathDict}$ in line 6.
...
\end{enumerate}
an index $k$ such that  $s_k$ is explored but the state $s_{k+1}$ is not explored must exist.  Now
...
at most $d+2$.  There are only finite number of states that have an estimated total distance less or
