...
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{start} := \pair(0,0)$ \quad and \quad $\mathtt{goal} := \pair(n,0)$.
\\[0.2cm]
...
\bigl\{\pair(a+1,0), \pair(a+1,b)\bigr\}  & \mbox{if $a < n$ and $b = n$} \\[0.1cm]
\bigl\{\pair(a,b+1)\bigr\}  & \mbox{if $a = n$ and $b < n$} \\[0.1cm]
\{\}  & \mbox{if $a = n$ and $b = n$} 
\end{array}
\right.
$
...
The heuristic is \blue{admissible} if it always underestimates the true distance, i.e.~if the function
\\[0.2cm]
\hspace*{1.3cm}
