...
\item Next, the $\mathtt{state}$ on top of the stack is removed from the stack.  If this $\mathtt{state}$ happens to be the
      $\mathtt{goal}$, a path has been found and is returned.
\item Otherwise, we check the distance $\mathtt{ds}$ of $\mathtt{state}$.  If this distance is greater than or equal to the
