...
\begin{Definition}[Search Problem]
  ...
  \item $Q$ is the set of \blue{states}, also known as the \blue{state space}.
        \index{state}
  \item \textts{nextStates} is a function taking a state as input and returning the set of those
        ...
        \\[0.2cm]
        The function \textts{nextStates} gives rise to the \blue{transition relation} $R$, which is a
  ...
  \end{enumerate}
  A \blue{path} \index{path} is a list $[s_1, \cdots, s_n]$ such that $s_{i+1} \in \mathtt{nextStates}(s_i)$ for all $i \in
  \{1,\cdots,n-1\}$.
  \index{path}
...
\item Lines 5 to 18 define the function \textts{nextStates}.  A state $s$ is represented as a triple of
      the form
      \\[0.2cm]
      \hspace*{1.3cm}
...
abstract notion that is needed in order to specify the function \texttt{nextStates}, but it is not implemented.
...
\item \textts{findTile} is an auxiliary function that is needed to implement the function \textts{nextStates}.
...
\item Given a $\mathtt{State}$, the procedure \textts{next\_states} computes the set of all states
...
\item Note that the recursive invocation of $\mathtt{dfs}$ returns $\mathtt{None}$ if the end of the
      ...
      line of the procedure $\mathtt{dfs}$ is reached, $\mathtt{None}$ is returned by default.
\end{enumerate}
...
The heuristic is \blue{admissible} if it never \underline{\color{red}{overestimates}} the true distance,
i.e.~if the function 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Hence, the heuristic is admissible iff it is \blue{optimistic}:  It must never overestimate the
...
\end{enumerate}
The function $\mathtt{aStarSearch}$ maintains 4 variables that are crucial for the understanding of the
algorithm.
\begin{enumerate}
...
\item Next, we have a \texttt{for} loop that iterates over all states neighbouring $\mathtt{state}$.
      ...
      \item For every $\mathtt{neighbour}$ we check the estimated length of a solution passing through
            ...
            $\mathtt{oldEstimate}$ is undefined, i.e.~it has the value $\mathtt{None}$, if we haven't yet encountered the node
      ...
      \item Next, we need to check whether this new solution that first passes through $\mathtt{state}$ and
            ...
            $\mathtt{oldEstimate}$, i.e.~the variable $\texttt{oldEstimate}$ might have the value $\mathtt{None}$.
            ...
            $\mathtt{neighbour}$ if it exists, which is the case if $\mathtt{oldEstimate}$ is different from $\mathtt{None}$.
      \end{enumerate}
\end{enumerate}
...
search shown in \myFig{bidirectional-bfs.stlx} relates to breadth first search shown in \myFig{Breadth-First-Search.ipynb}.
