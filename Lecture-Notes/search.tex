...
\end{enumerate}
Furthermore, we have used the fact that the information contained in the set $\textts{Visited}$ is already
...
\end{enumerate}
When we test the implementation shown above with the $3 \times 3$ sliding puzzle, it takes less than a second
on my computer to find a solution.  This is an improvement compared to breadth first search.
When it comes to memory consumption, we still have to maintain the set $\textts{Parent}$.  Fortunately, we will
...
However, there is also bad news: the solution that is found has a length of more than $10,000$ steps.  As the
...
For the $3 \times 3$ puzzle, the recursive implementation is not able to compute the solution with Python 3.8
because the kernel of the Jupyter notebook dies.  This had been different last year when I had used Python 3.7. 
Then, the solution had a length of more than $20\,000$ steps.  This, together with the fact that function
...
is the variable $\textts{Path}$.  If we can somehow keep the list $\textts{Path}$ short, then the recursive
version of depth first search uses only a small fraction of the memory needed by breadth first search.

\section{Iterative Deepening}
The fact that the stack-based version of depth first search took just one second to find a solution is very
...
\item The function $\textts{dls}$ implements a recursive version of depth first search but takes care to compute only
      those paths that have a length of at most $\textts{limit}$.  If the $\textts{Path}$ has reached a length
...
memory.  The reason is that the $\textts{Path}$ can never have a size that is longer than $\textts{limit}$ and
...
about 12 minutes.  There are two reasons for this:
\begin{enumerate}
...
$\textts{Path}$ of length $32$ that leads to the $\textts{goal}$ in if that $\textts{Path}$ has been found, the
...
\item $\textts{VisitedA}$ is the set of states that have been visited starting from $\textts{start}$, while
      $\textts{VisitedB}$ is the set of states that have been visited starting from $\textts{goal}$.
...
\item Then the \texttt{while} loop computes those states that can be reached from $\textts{FrontierA}$ and have not been
...
a path connecting $\textts{start}$ and $\textts{goal}$ while passing through the state $s$ would use.
...
the true distance of the state $\textts{goal}$ form the state $\textts{start}$, this implies that the path that
