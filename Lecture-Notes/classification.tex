...
One of the earliest applications of artificial intelligence is
...
is always a natural number, in this case the first feature would be a
 ...
 string ``\texttt{pharmacy}'' occurs once in an email containing but a hundred characters or whether it occurs
...
\item $y_i := \mathtt{classify}(o_i)$ \quad for all $i \in \{1, \cdots, n\}$
      ...
      i.e.~$y_i$ is the class of the $i$-th object.  These numbers are collected into the $n$-dimensional column vector $\mathbf{y}$.
\end{enumerate}
...
\item Line 7 starts the implementation of the natural logarithm of the sigmoid function, i.e.~we implement
      ...
      In fact, if we use \textsl{Python's} 64-bit double arithmetic, then even adding $1$ to $\exp(100)$
...
\item Line 13 prints the accuracy.  As Figure \ref{fig:exam-iq-boundary.pdf} shows, 3 examples have been
      miss-predicted.  Two of these examples were bound to be miss-predicted, but the fact that the student
...
\\[0.2cm]
\hspace*{0.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-06/04-Logistic-Regression-with-SciKit-Learn.ipynb}
{Artificial-Intelligence/Python/Chapter-06/04-Logistic-Regression-with-SciKit-Learn.ipynb}
...
\item Line 12 defines the function \texttt{logistic\_regression}.  This function takes 5 arguments.
      ...
      \item \texttt{X\_test} and \texttt{Y\_test} are the test data.
...
\\[0.2cm]
\hspace*{-0.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/tree/master/Python/Chapter-06/EmailData}{https://github.com/karlstroetmann/Artificial-Intelligence/tree/master/Python/Chapter-06/EmailData}
\\[0.2cm]
...
\blue{Support Vector Machines} (abbreviated as \textsc{SVM}s) had been invented in 1963 by Vladimir
...
\exercise
You are a landscape architect designing a rectangular fish pond for a client's garden.  To maximize the
happiness of the fish, your goal is to maximize the volume of the pond.  However, you are limited by two
specific material inventories left over from a previous job:
\begin{enumerate}
\item You have exactly 20 meters of expensive stone coping that must be used to create the border around the
      rim of the pond.  Hence the perimeter of the pond must be 20 meters.
\item You have exactly 48 square meters of high-grade rubber liner to cover the bottom and the vertical sides
      of the pond. \eox
\end{enumerate}
\textbf{Hint}: Somewhat surprisingly, the optimal pond is not square!
...
class of a negative example to be $-1$.  Let $y_i$ denote the class of example $\mathbf{x}^{(i)}$.
...
\end{equation}
Those vectors $\mathbf{x}^{(i)}$ that satisfy the \blue{equation}
\\[0.2cm]
\hspace*{1.3cm}
...
$\mathbf{w} \cdot \mathbf{x} + b = 0$.  To this end, assume that $\mathbf{x}_+$ is a \blue{positive support vector},
i.e.~a support vector belonging to the positive class.  Then we have
\begin{equation}
  \label{eq:svm-6}
  \mathbf{w} \cdot \mathbf{x}_+ + b = 1,  
\end{equation}
while $\mathbf{x}_-$ is a \blue{negative support vector}, i.e. a support vector belonging to the negative
...
That is very wrong what you are telling here.\\[0.2cm]
