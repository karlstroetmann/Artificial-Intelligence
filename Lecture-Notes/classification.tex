...
Formally, the classification problem in machine learning can be stated as follows:  We are given a
...
introduce \blue{logistic regression}, we will assume that the number of weights is the same as the number of
features.  Then, the weights specify the relative importance of the different features.
...
during the iterations.  Figure \ref{fig:gradient-ascent.stlx} on page \pageref{fig:gradient-ascent.stlx} shows
how this is done. 
...
\item The body of the \texttt{while} loop starting in line 5 executes one iteration of gradient ascent.
...
\item Next, we compute $\mathbf{x}_{n+1}$ in line 7 using the formula
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathbf{x}_{n+1} := \mathbf{x}_n + \alpha \cdot \nabla f(\mathbf{x}_n)$.
\item The corresponding value $f(\mathbf{x}_{n+1})$ is computed in line 8.
...
\begin{Verbatim}[ frame         = lines, 
    ...
    findMaximum := procedure(f, gradF, start, eps) {
        ...
        while (true) {
            ...
            if (fx < fOld) {   
                alpha   *= 0.5;
                [x, fx] := [xOld, fOld];  // reset values and
                continue;                 // start over
            } else {
                alpha *= 1.2;
            }
