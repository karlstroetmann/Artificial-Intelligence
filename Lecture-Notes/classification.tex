...
\\[0.2cm]
then the $j$-th feature is a \blue{binary feature}.  If we encode $\mathtt{false}$ as $0$ and $\mathtt{true}$ as
...
$f: \mathbb{R}^n \rightarrow \mathbb{R}$.
\\[0.2cm]
The basic idea is to take a vector $\mathbf{x}_0 \in \mathbb{R}^n$ as the start value and define a sequence of
...
\begin{array}{lcll}
  L'(t) & = & \ds \frac{S'(t)}{S(t)}                  & \mbox{by the chain rule} \\[0.5cm]
        ...
        & = & \ds 1 - S(t)                            & \mbox{by symmetry} \\[0.2cm]
        & = & \ds S(-t)
\end{array}
$
\\[0.2cm]
