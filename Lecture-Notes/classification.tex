...
\\[0.2cm]
\hspace*{1.3cm}
$\ds \mathtt{accuracy} := \lim\limits_{n\rightarrow\infty}\frac{\mathtt{card}\bigl(\{ o \in S_n \mid \mathtt{guess}(o) = \mathtt{classify}(o)\}\bigr)\;}{\mathtt{card}(S_n)}$.
\\[0.2cm]
...
Remember that the gradient of $f$ is defined as the vector
\\[0.2cm]
\hspace*{1.3cm}
$\nabla f := \left(
 \begin{array}{c}
...
\\[0.2cm]
Here, we have combined the different feature vectors $\mathbf{x}_i$ into the matrix $\mathbf{X}$ such that
$\mathbf{x}_i^\top$ is the $i$-th row of the matrix $\mathbf{X}$.  Since it is
...
$\ell(\mathbf{X},\mathbf{y},\mathbf{w})$.  This logarithm is called the \blue{log-likelihood}
\index{log-likelihood} and is defined as 
\\[0.2cm]
\hspace*{1.3cm}
...
gradient of the log-likelihood was not in vain:  We will use the method of gradient ascent in order to find
...
\item The next two lines are needed to extract the coefficients $\vartheta_0$, $\vartheta_1$, and $\vartheta_2$
      ...
      The \blue{decision boundary} are those values of $(h, q)$ such that $P(h) = \frac{1}{2}$.  This set of
...
\item In order to evalate the performance of this algorithm, we need to define two new concepts: \blue{precision} and 
