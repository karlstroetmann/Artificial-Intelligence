...
\begin{Definition}[Computational Graph]
  Assume a set $\{x_1, \cdots, x_k \}$ of \blue{input variables} is given.
  A \blue{computational graph} \index{computational graph} $G$ is a list of \blue{computational nodes}.
  \index{computational node}  There are four types of computational nodes:
  \begin{enumerate}
  \item A \blue{variable node} is a tuple of length 1 of the form
        \\[0.2cm]
        \hspace*{1.3cm}
        $\langle x, \rangle$
        \\[0.2cm]
        where $x$ is a variable from the set $\{x_1,\cdots,x_k\}$.
  ...
  \item A \blue{unary node} is a tuple of the form
        ...
        $\langle n, f, a\rangle$
        \\[0.2cm]
        where, again, $n$ is the \blue{name} of the node that is used as an auxiliary variable. $f$ is a unary
        function symbol from a set of unary functions.  In our examples $f$ will be a member of the set
        \\[0.2cm]
        \hspace*{1.3cm}
  ...
  \item A \blue{binary node} is a tuple of the form
        ...
        variable.
\end{enumerate}
The \blue{output variable} of G is the name of the variable that is defined in the last node.
\eoxs
\end{Definition}
...
In order to \blue{evaluate} an admissible computational graph $G$ that contains $n$ input variables, we will
assume that the first $n$ nodes are labelled with the variables $\mathtt{x}_1$, $\cdots$, $\mathtt{x}_n$ and
that the output variable of $G$ is \texttt{y}.  Furthermore, we need a dictionary \texttt{Values} that
...
\blue{adjoint}, which will be given later after we have defined the notion of a \blue{parent} of a node.  
\FloatBarrier
...
\begin{Definition}[Adjoint]
  Assume $G$ is a computational graph such that the output variable is \texttt{y}.
...
\item Next, we have to import the autograd version of the library \texttt{numpy}.  This version offers most, but
      ...
      to create this computational graph.
