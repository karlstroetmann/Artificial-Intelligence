...
of February 2024, the best version of the chess program
\href{https://en.wikipedia.org/wiki/Stockfish_(chess)}{Stockfish} runs 
on ordinary desktop computers and has an \href{https://en.wikipedia.org/wiki/Elo_rating_system}{Elo rating} of 3634.  
To compare, according to the
\href{https://ratings.fide.com/top.phtml?list=men}{Fide} list of February 2024, the current 
World Chess Champion \href{https://en.wikipedia.org/wiki/Magnus_Carlsen}{Magnus Carlsen} has an Elo rating of
2830.  If two players differ by more than 400 in their ELO ranking, the lower ranked player does not stand a
...
\item $\textts{utility}$ is a function that takes a state $s$ such that $s \in \textts{TerminalStates}$ as its argument.  It returns 
      the \blue{value} that the game has for the first player.  In general, a
...
\end{enumerate}
The implementation shown so far has one important drawback: Every state needs 706 bytes in memory.
