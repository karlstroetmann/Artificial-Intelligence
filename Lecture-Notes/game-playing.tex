...
\item $\textts{utility}$ is a function that takes a state $s$ as its argument.  If the game is finished, it returns 
      ...
      game, and if $\textts{utility}(s) = 0$, then the game is drawn.  Hence the signature of $\textts{utility}$ is
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item The function $\textts{next\_states}$ takes a $\textts{State}$ and a $\textts{player}$ and computes the list
      ...
      Note that we had to turn the \textts{State} into a list of lists in order to manipulate it.
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-03/Nim-Frame.ipynb}{
   Artificial-Intelligence/blob/master/Python/Chapter-03/Nim-Frame.ipynb}.
\\[0.2cm]
 Then,  test the game using the notebook
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-03/5-Alpha-Beta-Pruning.ipynb}{Artificial-Intelligence/blob/master/Python/Chapter-03/5-Alpha-Beta-Pruning.ipynb}.
\eox
...
%       \\[0.2cm]
%       \hspace*{1.3cm}
%       \href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-03/Tic-Tac-Toe.ipynb}{
%             Artificial-Intelligence/blob/master/Python/Chapter-03/Tic-Tac-Toe.ipynb}
%       \\[0.2cm]
%       from my GitHub page \textts{https://github.com/karlstroetmann}.
% \item Test the game using the notebook
%       \\[0.2cm]
%       \hspace*{1.3cm}
%       \href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-03/Game-Move-Ordering.ipynb}{
%            Artificial-Intelligence/blob/master/Python/Chapter-03/Game-Move-Ordering.ipynb}.
%       \eox
% \end{enumerate}
% \pagebreak


I have tested the program described so far with the game
\href{https://en.wikipedia.org/wiki/Connect_Four}{Connect Four}.  You can play this game online at 
\\[0.2cm]
\hspace*{1.3cm}
...
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/3
  Games/Connect-Four.ipynb}{Artificial-Intelligence/blob/master/Python/Chapter-03/Connect-Four.ipynb} 
\\[0.2cm]
The heuristic that I have implemented uses \blue{triple}s.  A \blue{triple} is defined as a set of three marks
of either \textts{X}s or \textts{O}s in a row that is followed by a blank space.  The blank space could also be
between the marks.  Now if there is a state $s$ that has $a$ triples of \textts{X}s and $b$ triples of
\textts{O}s and the game is not finished, then define 
\\[0.2cm]
\hspace*{1.3cm}
$\ds \textts{value}(s, \textts{X}, \textts{limit}, \alpha, \beta) = \frac{a - b}{10}$ \quad if $\textts{limit} = 0$.
\\[0.2cm]
In a similar way, pairs can be defined as a set of two marks of the same player.  These are valued with a
factor of $\frac{1}{100}$.  Using this heuristic, the resulting game engine is already quite strong when
looking 10 moves ahead.
...
% \hspace*{1.3cm}
% \href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/3
%   Games/Connect-Four-Frame.ipynb}{Artificial-Intelligence/blob/master/Python/Chapter-03/Connect-Four-Frame.ipynb} 
% \\[0.2cm]
