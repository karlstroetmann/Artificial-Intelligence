...
\item The function $\texttt{utility}$ takes a $\texttt{State}$ and a $\texttt{player}$.  If the game is 
      ...
      Given a $\texttt{State}$, a \blue{line-set} is a set of those positions that either form a horizontal,
      ...
      from the blank.  If this element is the same as $\texttt{player}$, then the game is \blue{won} by
      $\texttt{player}$, otherwise it is \blue{lost} for him.

      Finally, if there are no empty squares left, then the game is a \blue{draw}.
\item The auxiliary procedure $\texttt{all\_lines}$ takes a $\texttt{State}$ and computes the set $\texttt{Lines}$ of all
      ...
      \item Finally, the set
            ...
            \texttt{\{ State[idx][-idx] : idx in [1..3] \}}
            \\[0.2cm]
            is the rising diagonal.  This works, because for a list $\texttt{L}$ and a number $k$,
            the expression $\texttt{L}[-k]$ returns the $k$-th last element of $\texttt{L}$,
            i.e.~$\texttt{L}[-1]$ is the last element of $\texttt{L}$, $\texttt{L}[-2]$ is the second last
            element of $L$, and in general we have $\texttt{L}[-k] = \texttt{L}[\#\texttt{L} - (k-1)]$ for all 
            $k \in \{1,\cdots,\#\texttt{L}\}$.
      \end{enumerate}
...
\item Similarly, if $\texttt{value}(s, p) \geq \beta$, we require that the value
      ...
      \\[0.2cm]
      \hspace*{0.3cm}
      $\beta \leq \texttt{value}(s, p) \;\rightarrow\; \texttt{value}(s, p) \leq \texttt{alphaBeta}(s, p, \alpha, \beta)$.
\end{enumerate}
