...
\myFig{Minimax.ipynb} shows an implementation of this strategy. It also shows the function \textts{best\_move}.
...
\item The function $\mathtt{memoize}$ is a second order function that takes a function $f$ as its argument.
      ...
      to compute the result.  This result is then stored in the dictionary $\mathtt{gCache}$ before it is returned.
...
\item All that needs to be done is then to memoize both the function $\textts{maxValue}$ and the function
      $\mathtt{minValue}$.  In this case it is not a problem that these functions share the same dictionary
