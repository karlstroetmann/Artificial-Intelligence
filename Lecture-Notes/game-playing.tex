...
The definition of the function \textts{minValue} is defined by the following recursive equations
\begin{enumerate}
...
\item If $\textts{State}$ is a terminal state, the function returns the $\textts{utility}$ of the given
      $\textts{State}$.
\item If $\textts{alpha} > \textts{beta}$, the function returns the lower bound $\textts{alpha}$.
      In this case, we know that we can not have $\alpha \leq \textts{maxValue}(s) \leq \beta$.
      Instead, we have either $\alpha \leq \textts{maxValue}(s)$ or $\textts{maxValue}(s) < \alpha$.
      We analyse these cases separately.
      \begin{enumerate}[(a)]
      \item $\alpha \leq \textts{maxValue}(s)$

            Then, because $\beta < \alpha$ we also have $\beta < \textts{maxValue}(s)$ and
            we have
            \\[0.2cm]
            \hspace*{1.3cm}
            $\beta \leq \alpha = \textts{alphaBetaMax}(s)$,
            \\[0.2cm]
            as required.
      \item $\textts{maxValue}(s) < \alpha$.  As we have
            \\[0.2cm]
            \hspace*{1.3cm}
            $\textts{alphaBetaMax}(s) = \alpha \leq \alpha$
            \\[0.2cm]
            the specification is again satisfied.
      \end{enumerate}
\item The case $\alpha = \beta$ is similar to the case above.
      However, in that case we also have to deal with the case that $\alpha = \textts{maxValue}(s) = \beta$.
      However, since then $\textts{alphaBetaMax}(s) = \alpha = \textts{maxValue}(s)$, this subcase is trivial.
...
\item $f$ is either the function \textts{maxValue} or the function \textts{minValue}.
...
that are stored a pairs of the form $\langle \textts{flag}, v \rangle$, where $v$ is a value computed by the
...
\begin{enumerate}
\item $\texttt{gCache[State]} = (\quoted{=}, v) \rightarrow \texttt{evaluate(State)} = v$,

      i.e.~if the flag is equal to \quoted{=}, then the value stored in \textts{gCache[State]}
      is the exact value of the given \textts{State}.
\item $\texttt{gCache[State]} = (\quoted{\leq}, v) \rightarrow \texttt{evaluate(State)} \leq v$,

      i.e.~if the flag is equal to \quoted{\leq}, then the value stored in \textts{gCache[State]}
      is an upper bound for \textts{evaluate(State)}.

\item $\texttt{gCache[State]} = (\quoted{\geq}, v) \rightarrow \texttt{evaluate(State)} \geq v$,

      i.e.~if the flag is equal to \quoted{\geq}, then the value stored in \textts{gCache[State]}
      is a lower bound for \textts{evaluate(State)}.
\end{enumerate}
If \textts{gCache[State]} is defined, then the computation of `evaluate(State, alpha, beta)`
proceeds according to the following case distinction:
\begin{enumerate}
\item If the stored value $v$ is exact, we can return this value:
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{gCache[State]} = (\quoted{=}, v) \rightarrow \texttt{evaluate}(\texttt{State}, \alpha, \beta) = v$.
\item If the stored value $v$ is an upper bound and this upper bound is less or equal than $\alpha$, then we know that
      the true value of \textts{State} is less or equal than $\alpha$ and hence we can also return the value $v$:
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{gCache[State]} = (\quoted{\leq}, v) \wedge v \leq \alpha \rightarrow 
...
\item If the stored value $v$ is an upper bound and this upper bound is bigger than $\alpha$ but less than
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{gCache[State]} = (\quoted{\leq}, v) \wedge \alpha < v < \beta \rightarrow 
...
\item If the stored value $v$ is an upper bound and this upper bound is bigger or equal than $\beta$, then the
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{gCache[State]} = (\quoted{\leq}, v) \wedge \beta \leq v \rightarrow 
...
\item If the stored value $v$ is a lower bound and this lower bound is greater or equal than $\beta$, then we 
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{gCache[State]} = (\quoted{\geq}, v) \wedge \beta \leq v \rightarrow 
     \texttt{evaluate}(\texttt{State}, \alpha, \beta) = v$.
\item If the stored value $v$ is a lower bound and this lower bound is less than $\beta$ but bigger than $\alpha$,
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{gCache[State]} = (\quoted{\geq}, v) \wedge \alpha < v < \beta \rightarrow 
...
\item If the stored value $v$ is a lower bound and this lower bound is less or equal than $\alpha$, then the
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{gCache[State]} = (\quoted{\geq}, v) \wedge v \leq \alpha \rightarrow 
      \texttt{evaluate}(\texttt{State}, \alpha, \beta) = f(\texttt{State}, \alpha, \beta)$.
