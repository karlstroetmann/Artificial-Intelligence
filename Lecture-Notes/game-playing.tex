...
\item $\texttt{Players}$ is  the set of \blue{players} of the game.
...
In this chapter we will only consider so called \blue{two person zero sum games}.  This means that the set $\texttt{Players}$
...
\begin{Definition}[\blue{Game Tree}]
  ...
  \\[0.2cm]
  \hspace*{1.3cm}
  $[s_0, s_1, \cdots, s_n]$ \quad such that \quad $\forall i\in\{0,\cdots,n-1\}: s_{i+1} \in \texttt{nextStates}(s_i)$.
  \\[0.2cm]
  The \blue{game tree} of the game $\mathcal{G}$ is the set of all possible plays.  \eox
\end{Definition}
...
\item Derive a formula that gives the number of all states of simplified tic-tac-toe.  
      ...
      \textbf{Notice} that this question does not ask for the number of all terminal states but rather ask for
      all states. 
\item Write a short program to evaluate the formula derived in part (a) of this exercise.
\item Derive a formula to compute the number of all possible plays of simplified tic-tac-toe.
...
the given $\texttt{State}$ using a heuristic function.  This leads to the code shown in \myFig{game-limit.stlx}.
