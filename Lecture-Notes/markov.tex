...
\begin{Definition}[Markov Decision Process]
  ...
  \item $T$ is the transition probability.  For all states $s_1,s_2 \in S$ and every action $a \in A$,
        ...
        the system will be in state $s_2$ afterwards.  Formally, we have
        \\[0.2cm]
        \hspace*{1.3cm}
...
The Bellman equation is a fixpoint equation that determines the \emph{value of a state}, where the
...
is no discounting of future values.  If $\gamma < 1$, the Bellman equation can be solved by fixed
...
Instead of finding the values $V^*(s)$ of a given state $s$ we instead try to compute the value a state
$s$ has once an action $a$ for that state has been fixed.  These values are called Q-values and they
are denoted as $Q(s,a)$.  The Bellman equation for Q-values is
\\[0.2cm]
\hspace*{1.3cm}
...
$R(s,a,s')$ are given.  In Q-learning, we have an unknown Markov decision process and attempt to
...
$\alpha$ is close to $1$, the learning rate is high and the past is unimportant.  Usually, $\alpha$ is
...
Compute the Q-values via a number $m$ of different features in a linear way as follows:
\\[0.2cm]
\hspace*{1.3cm}
