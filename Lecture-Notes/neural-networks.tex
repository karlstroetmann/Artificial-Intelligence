...
\item Similarly to our treatment of logistic regression, we define a cost function that measures the quality of
...
\\[0.2cm]
Here, $f$ is called the \blue{activation function}.  \index{activation function} In our applications, we will
...
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-07/NN-Architecture.ipynb}{NN-Architecture.ipynb}
...
\item The second layer of neurons, which is the third layer of nodes,  is activated and computes the activation
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathbf{a}^{(3)}(\mathbf{x}) := f\bigl(W^{(3)} \cdot \mathbf{a}^{(2)}(\mathbf{x}) + \mathbf{b}^{(3)}\bigr)
                          = f\Bigl(W^{(3)} \cdot S\bigl(W^{(2)} \cdot \mathbf{x} + \mathbf{b}^{(2)}\bigr) + \mathbf{b}^{(3)}\Bigr)
        $
...
\item The computing power that is available today has vastly increased in the last 20 years.
      ...
      \href{https://www.nvidia.com/de-de/geforce/graphics-cards/50-series/rtx-5090/}{NVIDIA RTX 5090}
      graphic card offers 419 teraflops in single precision performance. It needs a power supply that
      ...
      White was about $110,000,000\,\symbol{36}$.   To compare, the \textsc{Nvidia} RTX 5090 costs $2,\!099\,\symbol{36}$.

      The \textsc{Nvidia} RTX 5090 is a consumer graphics card.  There are so called \blue{Tensor Core GPUs}
      like the \textsc{Nvidia} Blackwell B200 Tensor Core GPU that can achieve more than a
      petaflop when training neural networks.  Furthermore, these cards can be bundled
      in clusters.  However, a card containing two \textsc{Nvidia} B200 GPUs and one \textsc{Nvidia} Grace CPU
...
can be computed as shown below.  Note that we suppress the fact that $\mathbf{a}^{(L)}(\mathbf{x})$ is a
function of the input vector $\mathbf{x}$ in the following calculation.  The reason is that we are
computing the partial derivative of the cost function with respect to the weights $w_{j,k}^{(l)}$ and biases
$b_j^{(l)}$ and these weights and biases do not depend on the inputs $x_i$.  Therefore, it makes sense to
suppress the input vector $\mathbf{x}$ in the notation.
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}{lcll}
...
& = & \ds \frac{\partial \quad}{\partial z_j^{(L)}}  \frac{1}{2} \cdot \bigl(\mathbf{a}^{(L)} - \mathbf{y}\bigr)^2
      \\[0.5cm]
& = & \ds \frac{1}{2} \cdot \frac{\partial \quad}{\partial z_j^{(L)}} 
      \sum\limits_{i=1}^{m(L)} \Bigl(a_i^{(L)} - y_i\Bigr)^2
      \\[0.5cm]
& = & \ds \frac{1}{2} \cdot \frac{\partial \quad}{\partial z_j^{(L)}} 
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds z_j^{(l)} = \left(\sum\limits_{i=1}^{m(l)} w_{j,i}^{(l)} \cdot f\bigl(z_i^{(l-1)}\bigr)\right) + b_j^{(l)}$.
\\[0.2cm]
Hence we have
\\[0.2cm]
\hspace*{1.3cm}
...
collected for the purpose of training neural networks for image recognition, contains about 14 million images.
...
and \ref{fig:Digit-Regocnition.ipynb-4} on the following pages.
...
\item The function \texttt{load\_data} reads the file \texttt{mnist.pkl.gz}, uncompresses it,
      ...
      \item \texttt{test\_data} holds  $10\,000$ images.
...
\item The function $\mathtt{rndMatrix}(r, c)$ creates a matrix of shape $(r, c)$ that is filled with
      ...
      $1/c$.  This function is used to initialize the weight matrices.  It is important that not all weights
...
\item The method \texttt{sgd} implements \blue{stochastic gradient descent}.   It receives 5 arguments.
      ...
      \item \texttt{mbs} is the size of the mini-batches that are used in stochastic gradient descent.
            ...
            of 20 was slightly slower, but this parameter seems not to be critical.
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-07/Digit-Recognition-NN.ipynb}{Digit-Recognition-NN.ipynb}.
\\[0.2cm]
The file is located in the directory \texttt{Python/Chapter-07} in my
\href{https://github.com/karlstroetmann/Artificial-Intelligence}{github repository} for this lecture.
There is also a vectorized version of this notebook available at
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-07/Digit-Recognition-NN-vectorized.ipynb}{Digit-Recognition-NN-vectorized.ipynb}.
\\[0.2cm]
This version is about four times faster than the version discussed in this section.
