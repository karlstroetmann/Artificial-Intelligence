...
\href{https://en.wikipedia.org/wiki/G%C3%B6del%27s_completeness_theorem}{G\"{o}del's completeness theorem} 
...
\end{enumerate}
Since the existence of this infinite decreasing sequence contradicts the well-foundedness of $\prec$, our proof
is complete. \qed
...
\begin{Theorem}[\href{https://en.wikipedia.org/wiki/Transfinite_induction}{Transfinite Induction}]
  ...
  Assume the binary relation $\prec \;\subseteq\; M \times M$ is a well-founded partial ordering and $F(x)$ is some formula.
  If we have that
  \\[0.2cm]
  \hspace*{1.3cm}
...
\item $w(s) = w(t)$, $\texttt{count}(s, x) \leq \texttt{count}(t, x)$ for all variables $x$ occurring in $s$, and
...
\begin{Definition}[Composition of Substitutions] \index{Composition of Substitutions}
    Assume that \\[0.2cm]
    \hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$p(x_1, f(x_4))\sigma = p(x_2, f(x_4))$ \quad and \quad \\[0.2cm]
\hspace*{1.3cm}
...
\hspace*{1.0cm} $f(s_1,\cdots,s_m)\sigma = f(s_1\sigma,\cdots,s_m\sigma)$ \quad and \quad
...
\item Otherwise, call $R$ the set of oriented rewrite rules that result from orienting the equations in $E$
      into rewrite rules.
\item Compute all critical pairs that can be built from equations in $R$.
      \begin{enumerate}
...
However, note that we have to rename the variables in the second instance of the rule to prevent accidental name
...
\\[0.2cm]
Since $i(a) \circ i(y) \prec i(x \circ a)$, we have found the new rule
\\[0.2cm]
\hspace*{1.3cm}
