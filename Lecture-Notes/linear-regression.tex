...
\begin{equation}
 ...
 $\ds\bigl(X^\textrm{T} \cdot X\bigr) \cdot \mathbf{w} = X^\textrm{T} \cdot \mathbf{y}$.}}}} 
\end{equation}
...
\item The data from this object is stored as a list of lists in $\mathtt{data}$.
...
\item The normal equation is then solved using the function $\mathtt{la\_solve}$.
...
When we run the general linear regression for the data stored in \texttt{cars.csv} that has been discussed
previously, the proportion of explained variance is $81 \mathtt{\symbol{37}}$.  Considering that our data does
...
\item The fifth column gives the number of refrigerators that have been sold in the corresponding store.
\end{enumerate}
Your task as a marketing manager is to evaluate the effectiveness of the different type of advertisements.
Should you spend more money on newspaper adds and less on TV adds or is it the other way around?  Or are radio
...
\cite{james:2014}.  As far as I understand it, the data contained in this file is hypothetical data, i.e.~it
has been made up. 
\eox
