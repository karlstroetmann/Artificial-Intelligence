...
engine displacement, affect fuel consumption. For instance, the \blue{weight} of a car and its
...
assume that the car's mass has a  significant influences on its fuel consumption. To model such complex
...
train the model on the remaining $k-1$ parts.  If the training set is small, $k$ will be either $2$ or $3$. We
use the removed part to try to find the best value for our hyper parameter by trial and error.  We do this for
all of the $k$ parts.  In each case we might find a sightly different optimal value $\lambda_k$.  Finally, we
take the mean $\bar{\lambda}$ of all values $\lambda_k$ that we have found in our $k$ experiments.
