...
\item The coefficient $\mathtt{theta1}$ is computed according to Equation \ref{eq:theta1}, which is repeated
      ...
      by subtracting \texttt{xMean} from every entry of \texttt{X}.
...
\\[0.2cm]
\hspace*{0.8cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-05/Random-Walk-Frame.ipynb}{
   Artificial-Intelligence/blob/master/Python/Chapter-05/Random-Walk-Frame.ipynb}.
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-05/6-Linear-Regression.ipynb}{\texttt{https://github.com/karlstroetmann/Artificial-Intelligence/\\
\hspace*{2.2cm}
 blob/master/Python/Chapter-05/6-Linear-Regression.ipynb}}.
...
\begin{figure}[!ht]
  \begin{center}
    \epsfig{file=Figures/gehaltstabelle.png, scale=0.50}    
  \end{center}

\caption{Salary table for civil servants in Germany.}
\label{fig:gehaltstabelle.png}
\end{figure}
...
The pay grades range from A3 to A16.  The monthly salaries are shown in Figure \ref{fig:gehaltstabelle.png} on
page \pageref{fig:gehaltstabelle.png}.  The columns correspond to the length of service (\emph{Dienstalter}).
A plot of the data for a given pay grade with the highest length of service are shown in Figure \ref{fig:gehaltsplot.png}
on page \pageref{fig:gehaltsplot.png}.

\begin{figure}[!ht]
  \begin{center}
    \epsfig{file=Figures/gehaltsplot.png, scale=0.32}    
  \end{center}

\caption{Salaries for civil servants w.r.t.~to pay grade.}
\label{fig:gehaltsplot.png}
\end{figure}

Typically, the salary gaps widen as one moves up the hierarchy—the jump
...
# Maximum Monthly Salary in Euro 
salary_list = [
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-05/trees.csv}{\texttt{Artificial-Intelligence/blob/master/Python/Chapter-05/trees.csv}},
\\[0.2cm]
...
\item The first column of this \textsc{Csv} file contains the diameter of these trees at a height of 54 inches above
      the ground.
\item The second column lists the heights of these trees in feet.
...
\\[0.2cm]
\hspace*{0.3cm}
\href{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-05/nba.csv}{\texttt{https://github.com/karlstroetmann/Artificial-Intelligence/blob/master/Python/Chapter-05/nba.csv}},
\\[0.2cm]
...
and observe the divergence between \blue{training performance} and \blue{test performance}. 
...
\label{fig:preprocessing-hitters}
\end{figure}

We discuss the important lines of the code fragment in Figure \ref{fig:preprocessing-hitters}.
\begin{enumerate}
\item Line 4 creates a \blue{data frame} containing the data from the file \texttt{Hitters.csv}.
\item Line 5 removes the column with the title ``\texttt{rownames}'' as this column contains the names of the
      players.
\item Line 6 removes those columns where the attribute ``\texttt{Salary}'' is missing.
\item Line 7 is the most important line.  Some of the columns have non-numerical entries.  For example,
      there is a column with the attribute ``\texttt{League}''.  There are two leagues in American baseball:
      \begin{enumerate}[(a)]
      \item The \blue{American League} is often referred to as the ``Junior Circuit''. 
      \item The \blue{National League} was historically the first baseball league and is therefore
            referred to as the ``Senior Circuit''.
      \end{enumerate}
      In the hitters dataset, the values are represented as the values ``\texttt{A}'' and ``\texttt{N}''.
      Furthermore, there are three divisions in American baseball:
      \begin{enumerate}
      \item The ``\texttt{East Divison}'',
      \item the ``\texttt{Central Divison}'',
      \item the ``\texttt{West Divison}'' divison.
      \end{enumerate}
      In the hitters dataset, the values are represented as the values ``\texttt{E}'', ``\texttt{C}'', and ``\texttt{W}''.

      The purpose of the method \texttt{pd.get\_dummies} is to transform these attributes into numerical
      values.  This is done using \blue{one hot encoding}\index{one hot encoding}.  We explain how this
      procedure works by example of the attribute ``\texttt{Divison}''.  First, for each of the three values of
      this attribute \texttt{pd.get\_dummies} adds a column to the dataframe \texttt{df} that encodes, whether
      the corresponding attribute is set.  Therefore the new dataframe will have three new columns with the attributes
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{Divison\_E}, \texttt{Divison\_C}, and \texttt{Divison\_W}.
      \\[0.2cm]
      If the value of the attribute \texttt{Divison} was ``\texttt{C}'' for a player, then the values of
      these new attributes are set to the binary values
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{0}, \texttt{1}, and \texttt{0}
      \\[0.2cm]
      respectively, because the player is not in the East Divison, hence $\texttt{Divison\_E} = 0$,
      he is in the Central Divison, hence $\texttt{Divison\_C} = 1$, and he is not in the West Divison, hence
      $\texttt{Divison\_W} = 0$.

      Observe that the column ``\texttt{Divison\_E}'' is redundant.  Once we know whether a player is in the
      West Divison or in the Central Divison, we know he can't be in the East Divison.  Similarly, if a player
      is neither in the West Divison nor in the Central Divison, he must be in the East Divison.  Therefore,
      the column ``\texttt{Divison\_E}'' is automatically dropped by the method \texttt{pd.get\_dummies},
      because we have added the argument ``\texttt{drop\_first=True}'' to the call of the method
      \texttt{df.get\_dummies}.

      Actually, in the data set there is no player from the Central Divison.  Therefore, there is only one
      divison and and hence the resulting data frame only contains the attribute \texttt{Divison\_W}.
      I have included the discussion above only to illustrate the concept of \emph{one hot encoding}.
      
      The same transformation is performed for the attribute ``\texttt{League}''.  Since there are only two
      values for this attribute, the attribute  ``\texttt{League}'' is replaced by the attribute
      ``\texttt{League\_N}'' which is $1$ if the player is playing in the National League and $0$ if he is
...
\remark
There is an interesting book with the title
\\[0.2cm]
\hspace*{1.3cm}
\href{https://wwnorton.com/books/Moneyball/}{Moneyball: The Art of Winning an Unfair Game} 
\\[0.2cm]
by Michael Lewis \cite{lewis:2003}.  The book follows Billy Beane, the General Manager  of the Oakland Athletics baseball
team. Faced with a tight budget and unable to compete with wealthy teams like the New York Yankees, Beane
adopted a radical new approach to scouting and analyzing players. This books has been turned into the film
\href{https://www.rottentomatoes.com/m/moneyball}{Moneyball}.

Beane and his assistant, Paul DePodesta, relied on
\href{https://sabr.org/sabermetrics}{sabermetrics}—specialized baseball statistics—rather than 
traditional scouting methods (like a player's appearance or raw athleticism).
The term \blue{sabermetrics} was coined by Bill James \cite{james:1986}, deriving its name from the acronym for the
Society for American Baseball Research (SABR). James defined the field as ``the search for objective knowledge
about baseball'', utilizing empirical statistical analysis to measure player performance more accurately than
traditional methods. 

They discovered that the market undervalued skills like the ability to get on base (On-Base Percentage or OBP)
and overvalued traditional stats like Stolen Bases or Batting Average. 
This allowed the Oakland Athleticis to buy players who were statistically effective but ignored by other teams because
they were older, injured, or looked ``funny'' when they played. Despite having one of the lowest payrolls in
Major League Baseball, the Oakland Athleticis became one of the most successful teams, famously achieving a 20-game winning
streak in 2002. 
